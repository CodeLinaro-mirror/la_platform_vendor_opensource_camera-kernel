...
static int __cam_isp_send_pause_resume_cmd_to_sensor(void *priv, void *data)
{
	...
	struct sensor_pause_data   sensor_pause_data;
	struct sensor_resume_data  sensor_resume_data;
	int rc = 0;
	...
	case CAM_ISP_CTX_PAUSE_CMD:
		sensor_pause_data.isp_dropped_req = event_notify_payload->request_id;
		sensor_pause_data.link_hdl = ctx->link_hdl;

		CAM_DBG(CAM_ISP, "pause cmd to sensor for ctx: %d link :0x%x",
			ctx->ctx_id, ctx->link_hdl);

		cam_subdev_notify_message(sensor_device_type,
			CAM_SUBDEV_MESSAGE_SENSOR_PAUSE, (void *)&sensor_pause_data);
		ctx->last_flush_req = sensor_pause_data.last_applied_req;
		ctx_isp->vcdt.vc = sensor_pause_data.last_stream_vc;
		ctx_isp->vcdt.dt = sensor_pause_data.last_stream_dt;
		ctx_isp->vcdt.frame_duration = sensor_pause_data.frame_duration;
		CAM_DBG(CAM_ISP,
			"ctx:%d vc :%d dt:%d sensor last-applied_req :%lld frame_duration:%lld",
			ctx->ctx_id, sensor_pause_data.last_stream_vc,
			sensor_pause_data.last_stream_dt,
			sensor_pause_data.last_applied_req, sensor_pause_data.frame_duration);
		break;
	case CAM_ISP_CTX_RESUME_CMD:
		sensor_resume_data.ife_applied_req = event_notify_payload->request_id;
		sensor_resume_data.link_hdl = ctx->link_hdl;

		CAM_DBG(CAM_ISP, "resume cmd : req :%lld ctx: %d link :0x%x",
			event_notify_payload->request_id, ctx->ctx_id, ctx->link_hdl);
		cam_subdev_notify_message(sensor_device_type,
			CAM_SUBDEV_MESSAGE_SENSOR_RESUME, (void *)&sensor_resume_data);

		if (ctx_isp->sensor_req_info.last_applied_req !=
			sensor_resume_data.sensor_applied_req_id) {
			ctx_isp->sensor_req_info.prev_applied_req =
				ctx_isp->sensor_req_info.last_applied_req;
		}

		if (sensor_resume_data.sensor_applied_req_id) {
			ctx_isp->sensor_req_info.last_applied_req =
				sensor_resume_data.sensor_applied_req_id;
		}
		if (ctx_isp->sensor_req_info.last_applied_req >=
	...
	struct cam_context *ctx, struct cam_config_dev_cmd *cmd)
{
	...
	if (ctx->state < CAM_CTX_ACTIVATED &&
		...
		CAM_ERR(CAM_ISP, "acq type: %d, is_slave_down: %d ctx:%u",
