	...
	void *hw_cfg_args)
{
	...
	for (i = 0; i < sensor_grp_config->num_grp_cfg; i++) {
		...
		if (stream_grp_cfg->stream_cfg_cnt > CAM_ISP_STREAM_CFG_MAX) {
			CAM_ERR(CAM_ISP,
				"stream cfg cnt %u exceeds max %u for grp idx:%d",
				stream_grp_cfg->stream_cfg_cnt,
		...
		for (j = 0; j < stream_grp_cfg->stream_cfg_cnt; j++) {
			...
			if (grp_cfg->stream_cfg_cnt >= CAM_ISP_STREAM_CFG_MAX) {
				CAM_ERR(CAM_ISP,
					"stream cfg cnt %u exceeds max %u while updating grp idx:%u",
					grp_cfg->stream_cfg_cnt, CAM_ISP_STREAM_CFG_MAX, grp_idx);
				rc = -EFAULT;
				goto err;
			}
	...
	CAM_MEM_FREE(sensor_grp_config);
	return rc;
err:
	...
	int  index)
{
	...
	for (i = 0; i < in_port->num_out_res; i++) {
		...
		rdi_csid_acquire.tasklet = ife_ctx->common.tasklet_info;
	...
	int  index)
{
	...
	uint32_t                                  idx;
	uint32_t  i, vfe_in_res_id, num_out_res;
...
*/
		...
		vfe_acquire.vfe_out.vfe_res_out_id = res_id;
	...
	int   index)
{
	...
	for (i = 0; i < num_out_res; i++) {
		...
		if ((per_port_acquire) && (!in_port->ipp_count)) {
			out_port = &in_port->data[0];
		 ...
		 */
		if (((in_port->major_ver == 3) || (in_port->major_ver == 4)) &&
			...
			if (ife_ctx->vfe_out_map[k] != 0xff) {
				...
				if ((ife_out_res->res_id == res_type) &&
		...
		for (j = 0; j < CAM_ISP_HW_SPLIT_MAX; j++) {
			...
			if (j == CAM_ISP_HW_SPLIT_LEFT) {
				...
				if (((in_port->major_ver == 3) || (in_port->major_ver == 4)) &&
					vfe_acquire.vfe_out.use_hw_ctxt)
...
static inline void cam_ife_mgr_acquire_get_feature_flag_params_v4(
	struct cam_isp_in_port_info_v4      *in,
	struct cam_isp_in_port_generic_info *in_port)
{
	in_port->dynamic_sensor_switch_en = in->feature_mask & CAM_ISP_DYNAMIC_SENOR_SWITCH_EN;
	in_port->can_use_lite             = in->feature_mask & CAM_ISP_CAN_USE_LITE_MODE;
...
static int cam_ife_mgr_acquire_get_unified_structure_v4(
	struct cam_isp_acquire_hw_info *acquire_hw_info,
	uint32_t offset, uint32_t *input_size,
	struct cam_isp_in_port_generic_info *in_port)
{
	struct cam_isp_in_port_info_v4 *in = NULL;
	uint32_t in_port_length = 0;
	int32_t rc = 0, i;
	size_t len = 0;
	uint8_t log_buf[200];

	in = (struct cam_isp_in_port_info_v4 *)
		((uint8_t *)&acquire_hw_info->data +
		 acquire_hw_info->input_info_offset + *input_size);

	in_port_length = sizeof(struct cam_isp_in_port_info_v4) +
		(in->num_out_res - 1) *
		sizeof(struct cam_isp_out_port_info_v3);
	...
	*input_size += in_port_length;
	...
	cam_ife_mgr_acquire_get_feature_flag_params_v4(in, in_port);
	...
	struct cam_isp_in_port_generic_info *in_port)
{
	...
	case 3:
		return cam_ife_mgr_acquire_get_unified_structure_v3(
			acquire_hw_info, offset, input_size, in_port);
	case 4:
		return cam_ife_mgr_acquire_get_unified_structure_v4(
			acquire_hw_info, offset, input_size, in_port);
	default:
...
static int cam_ife_mgr_acquire_hw(void *hw_mgr_priv, void *acquire_hw_args)
{
	...
	for (i = 0; i < acquire_hw_info->num_inputs; i++) {
		...
		if (((in_port[i].major_ver == 3) || (in_port[i].major_ver == 4)) &&
			in_port[i].ipp_count)
	...
	uint32_t                   stop_cmd)
{
	...
	cam_ife_mgr_csid_stop_hw(ctx,
		...
		master_base_idx, stop_cmd, false);
	...
	for (i = 0; i < hfr_config->num_ports; i++) {
		...
		if (res_id_out >= out_max) {
			CAM_ERR(CAM_ISP, "invalid out restype:%x, ctx_idx: %u out_max:%x",
				port_hfr_config->resource_type, ctx->ctx_index, out_max);
			return -EINVAL;
		}
