...
skip_core_cfg:
	...
	rsrc_data->stored_irq_masks[VFE_TOP_VER4_SOF_MASK][CAM_IFE_IRQ_CAMIF_REG_STATUS0] =
		sof_irq_mask[CAM_IFE_IRQ_CAMIF_REG_STATUS0];

	rsrc_data->stored_irq_masks[VFE_TOP_VER4_FRAME_IRQ_MASK]
		[rsrc_data->common_reg->frame_timing_irq_reg_idx] =
...
skip_frame_irq_subscribe:
	...
	if (!rsrc_data->frame_irq_handle) {
		...
		rsrc_data->sof_irq_handle = cam_irq_controller_subscribe_irq(
			rsrc_data->vfe_irq_controller,
			CAM_IRQ_PRIORITY_1,
			rsrc_data->stored_irq_masks[VFE_TOP_VER4_SOF_MASK],
			vfe_res,
			vfe_res->top_half_handler,
			vfe_res->bottom_half_handler,
			vfe_res->tasklet_info,
