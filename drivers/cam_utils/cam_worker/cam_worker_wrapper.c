	...
	enum cam_worker_wrapper_type           worker_type)
{
	...
	if (!init_args) {
		CAM_ERR(CAM_ISP, "NULL for worker create args");
		return -EINVAL;
	}

	if (!init_args->worker_ctx) {
		CAM_ERR(CAM_ISP, "NULL for worker ctx in create args");
		return -EINVAL;
	}

	CAM_DBG(CAM_ISP, "Worker Wrapper Create process, worker type: %d",
		worker_type);
	switch (worker_type) {
	case WORKER_TYPE_WORKQ:
		...
		rc = cam_workq_create(init_args->u.workq_create_para.name,
			...
			cam_workq_process);
		if (rc < 0)
			CAM_ERR(CAM_ISP, "Workq Create failed, worker name: %s",
	...
	case WORKER_TYPE_TASKLET:
		...
		if (rc < 0)
			CAM_ERR(CAM_ISP, "Tasklet Init failed");
	...
	case WORKER_TYPE_KTHREAD:
		...
		if (rc)
			CAM_ERR(CAM_ISP, "Kthread creation failed, ,worker name: %s",
				init_args->u.kthread_create_para.name);

		break;
	default:
		CAM_ERR(CAM_ISP, "Invalid wrapper worker type: %d when worker create",
			worker_type);
		rc = -EINVAL;
		break;
	}
	...
	enum cam_worker_wrapper_class_type   worker_class_type)
{
	...
	if (!worker_wrapper_init_para) {
		CAM_ERR(CAM_ISP, "NULL for worker init parameters");
		return -EINVAL;
	}

	CAM_DBG(CAM_ISP, "Worker Wrapper Init, class type: %d, worker name: %s",
	...
	if (worker_class_type == WORKER_CLASS_RT) {
		...
		CAM_ERR(CAM_ISP, "Invalid Worker Class Type: %d, worker name: %s",
			worker_class_type, worker_wrapper_init_para->name);
		return -EINVAL;
	}

	worker_ctx_temp = kzalloc(sizeof(struct cam_worker_wrapper_ctx), GFP_KERNEL);
	if (!worker_ctx_temp) {
		CAM_ERR(CAM_ISP, "Failed to allocated worker wrapper, worker name: %s",
	...
	if (rc < 0) {
		CAM_ERR(CAM_ISP, "Worker Create Failed, type: %d, worker name: %s",
			worker_type, worker_wrapper_init_para->name);
		kfree(worker_ctx_temp);
		return rc;
	}
...
{
	CAM_DBG(CAM_ISP, "Worker Wrapper Destroy process: %d",
	...
	case WORKER_TYPE_KTHREAD:
		...
		break;
	default:
		CAM_ERR(CAM_ISP, "Invalid worker type: %d when worker destroy",
			worker_ctx->worker_type);
		break;
	}
...
void cam_worker_wrapper_deinit(void *worker_ctx_priv)
{
	...
	if (!worker_ctx) {
		CAM_ERR(CAM_ISP, "NULL for worker info");
		return;
	}
	...
	void *taskdata)
{
	...
	if (!worker_get_args) {
		CAM_ERR(CAM_ISP, "NULL worker ctx for get");
		return -EINVAL;
	}

	if (!taskdata_args) {
		CAM_ERR(CAM_ISP, "NULL taskdata for get");
		return -EINVAL;
	}
	...
	case WORKER_TYPE_KTHREAD:
		...
		break;
	default:
		CAM_ERR(CAM_ISP, "Invalid worker type: %d when get worker",
	...
	if (rc)
		CAM_ERR(CAM_ISP, "Error occur when get taskdata for worker: type[%d], rc[%d]",
	...
	CAM_IRQ_HANDLER_BOTTOM_HALF  bottom_half_handler)
{
	...
	case WORKER_TYPE_WORKQ:
		...
		if (rc)
			CAM_ERR(CAM_ISP, "worker enqueue for workq failed");
		break;
	case WORKER_TYPE_TASKLET:
	...
	case WORKER_TYPE_KTHREAD:
		...
		if (rc)
			CAM_ERR(CAM_ISP, "worker enqueue for kthread failed");
		break;
	default:
		CAM_ERR(CAM_ISP, "Invalid worker type: %d when enqueue worker task",
...
int cam_worker_wrapper_start(void *worker_ctx_priv)
{
	...
	switch (worker_ctx->worker_type) {
	case WORKER_TYPE_WORKQ:
		CAM_DBG(CAM_ISP, "Current bottom half does not support start, worker type: %d",
			worker_ctx->worker_type);
		break;
	case WORKER_TYPE_TASKLET:
	...
	case WORKER_TYPE_KTHREAD:
		...
		break;
	default:
		CAM_ERR(CAM_ISP, "Invalid Worker Type: %d during start",
			worker_ctx->worker_type);
		rc = -EINVAL;
		break;
	}
...
void cam_worker_wrapper_flush(void *worker_ctx_priv)
{
	...
	if (!worker_ctx) {
		CAM_ERR(CAM_ISP, "Invalid worker ctx priv");
		return;
	}

	CAM_DBG(CAM_ISP, "Worker Wrapper flush, worker type: %d", worker_ctx->worker_type);
	switch (worker_ctx->worker_type) {
	case WORKER_TYPE_WORKQ:
		cam_workq_flush(worker_ctx->u.workq);
		break;
	case WORKER_TYPE_TASKLET:
		CAM_DBG(CAM_ISP, "Current bottom half does not support flush, worker type: %d",
			worker_ctx->worker_type);
	...
	case WORKER_TYPE_KTHREAD:
		...
		break;
	default:
		CAM_ERR(CAM_ISP, "Invalid worker type: %d when worker flush",
			worker_ctx->worker_type);
		break;
	}
}
	...
	struct cam_worker_wrapper_taskdata_args *taskdata)
{
	...
	if (!worker_ctx || !taskdata_args) {
		CAM_ERR(CAM_ISP, "NULL taskdata or worker_ctx for get payload");
		return NULL;
	}
	...
	case WORKER_TYPE_WORKQ:
		...
		break;
	case WORKER_TYPE_TASKLET:
		CAM_DBG(CAM_ISP, "No task paylaod for current worker: %d",
			worker_ctx->worker_type);
		break;
	case WORKER_TYPE_KTHREAD:
		...
		break;
	default:
		CAM_ERR(CAM_ISP, "Invalid Worker Type: %d when get payload",
			worker_ctx->worker_type);
		break;
	}
	...
	int                                      work_data_idx)
{
	...
	if (!worker_ctx) {
		CAM_ERR(CAM_ISP, "NULL worker ctx for payload bind");
		return -EINVAL;
	}
	...
	case WORKER_TYPE_WORKQ:
		...
		break;
	case WORKER_TYPE_TASKLET:
		CAM_ERR(CAM_ISP, "Invalid payload bind, worker type: %d",
			worker_ctx->worker_type);
		rc = -EINVAL;
		break;
	case WORKER_TYPE_KTHREAD:
		...
		break;
	default:
		CAM_ERR(CAM_ISP, "Invalid Worker Type: %d when payload bind",
			worker_ctx->worker_type);
		rc = -EINVAL;
		break;
	}
	...
	struct cam_worker_wrapper_mini_dump *worker_dump_info)
{
	...
	if (!worker_ctx || !worker_dump_info) {
		CAM_ERR(CAM_ISP, "Dump worker failed due to invalid worker_ctx or dump struct");
		return;
	}
	...
	case WORKER_TYPE_WORKQ:
		...
		break;
	case WORKER_TYPE_TASKLET:
		CAM_DBG(CAM_ISP, "No support for tasklet info dump");
		break;
	case WORKER_TYPE_KTHREAD:
		worker_dump_info->worker_scheduled_ts =
...
int cam_worker_wrapper_prop_update_init(void)
{
	...
	if (!IS_KTHREAD_IN_USE) {
		CAM_INFO(CAM_ISP, "Kthread is not in use, not available to tune property");
		return -EOPNOTSUPP;
	}

	rc = cam_kthread_property_update_init();
	if (rc)
		CAM_ERR(CAM_ISP, "Failed at setting up kthread property update, rc: %d", rc);
