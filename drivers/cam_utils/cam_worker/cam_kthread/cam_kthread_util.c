...
static void cam_kthread_put_task(struct cam_kthread_task *task)
{
	...
	CAM_DBG(CAM_ISP, "PUT task kthread %s, free_cnt %d",
...
inline void cam_kthread_flush(struct cam_core_kthread *kthread)
{
	...
	if (!kthread) {
		CAM_ERR(CAM_ISP, "kthread is null");
		return;
	}
...
void cam_kthread_process(struct kthread_work *w)
{
	...
	if (!w) {
		CAM_ERR(CAM_ISP, "NULL task pointer for kthread work");
		return;
	}
...
struct cam_kthread_task *cam_kthread_get_task(struct cam_core_kthread *kthread)
{
	...
	CAM_DBG(CAM_ISP, "GET task kthread %s, free_cnt %d",
		kthread->worker_name, atomic_read(&kthread->task.free_cnt));
	...
	struct cam_kthread_task *kthread_task)
{
	...
	if (!kthread) {
		CAM_ERR(CAM_ISP, "Invalid kthread");
		return NULL;
	}

	if (!kthread_task) {
		CAM_ERR(CAM_ISP, "Invalid kthread task, worker name: %s",
			kthread->worker_name);
		return NULL;
	}
	...
	void *priv, int32_t prio)
{
	...
	if (!task) {
		CAM_WARN(CAM_ISP, "Invalid task pointer, can not schedule");
		return -EINVAL;
	}
	...
	CAM_DBG(CAM_ISP, "Enqueue kthread task %pK, pending_cnt %d, worker name: %s",
...
abort:
	cam_kthread_put_task(task);
	CAM_INFO(CAM_ISP, "Task aborted and queued back to pool, worker name: %s",
		kthread->worker_name);
	return rc;
}
	...
	struct cam_core_kthread **kthread, enum cam_kthread_context in_irq)
{
	...
	if (in_irq >= CAM_KTHREAD_USAGE_MAX || num_tasks <= 0) {
		CAM_ERR(CAM_ISP, "Invalid kthread arguments, in_irq: %d, num_tasks: %d",
			in_irq, num_tasks);
		return -EINVAL;
	}

	cam_kthread = kzalloc(sizeof(struct cam_core_kthread), GFP_KERNEL);
	if (!cam_kthread) {
		CAM_ERR(CAM_ISP, "Failed at allocating kthread worker");
		return -ENOMEM;
	}

	strlcat(buf, name, sizeof(buf));
	CAM_DBG(CAM_ISP, "Create kthread %s, num_task: %d, kthread context: %d",
		buf, num_tasks, in_irq);

	cam_kthread->job = kthread_create_worker(0, buf);
	if (IS_ERR(cam_kthread->job)) {
		rc = PTR_ERR(cam_kthread->job);
		CAM_ERR(CAM_ISP, "Kthread job creation failed, err code: %d", rc);
	...
	cam_kthread->task.pool = kcalloc(cam_kthread->task.num_task,
	...
	*kthread = cam_kthread;
	...
	kthread_data = kzalloc(sizeof(struct cam_kthread_data), GFP_KERNEL);
	if (!kthread_data) {
		CAM_ERR(CAM_ISP, "Failed at allocating memory for kthread data");
		rc = -ENOMEM;
...
void cam_kthread_destroy(struct cam_core_kthread **cam_kthread)
{
	...
	if (!cam_kthread || !*cam_kthread) {
		CAM_ERR(CAM_ISP, "Invalid argument for kthread destroy");
		return;
	}

	kthread = *cam_kthread;
	CAM_DBG(CAM_ISP, "Destroy kthread %s", kthread->worker_name);
...
static int cam_kthread_update_thread_property_dbg(struct inode *inode, struct file *flip)
{
	...
	if (IS_ERR_OR_NULL(task)) {
		CAM_ERR(CAM_ISP, "No empty task = %ld in prop update worker", PTR_ERR(task));
		return -ENOENT;
	}
	...
	if (rc) {
		CAM_ERR(CAM_ISP, "Could not enqueue task, rc: %d", rc);
		return rc;
	}
	...
	if (g_cam_kthread_info.result)
		CAM_ERR(CAM_ISP, "Failed to set kthread properties, result: %d",
	...
	size_t size, loff_t *loff_t)
{
	...
	*delimiter4 = '\0';
	...
	CAM_INFO(CAM_ISP, "Failed at setting debugging thread property");
	return -EINVAL;
}
