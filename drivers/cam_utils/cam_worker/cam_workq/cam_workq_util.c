	...
	struct cam_workq_task *workq_task)
{
	...
	if (!workq) {
		CAM_ERR(CAM_ISP, "Invalid workq");
		return NULL;
	}

	if (!workq_task) {
		CAM_ERR(CAM_ISP, "Invalid workq task, worker name: %s",
			workq->workq_name);
		return NULL;
	}
...
static int cam_workq_process_task(struct cam_workq_task *task)
{
	...
	if (task->process_cb)
		...
		CAM_WARN(CAM_ISP, "FATAL:no task handler registered for workq");
...
void cam_workq_process(struct work_struct *w)
{
	...
	if (!w) {
		CAM_ERR(CAM_ISP, "NULL task pointer can not schedule");
		return;
	}
	...
	void *priv, int32_t prio)
{
	...
	if (!task) {
		CAM_WARN(CAM_ISP, "NULL task pointer can not schedule");
		return -EINVAL;
	}

	workq = (struct cam_core_workq *)task->parent;
	if (!workq) {
		CAM_DBG(CAM_ISP, "NULL workq pointer suspect mem corruption");
		return -EINVAL;
	}
	...
	CAM_DBG(CAM_ISP, "enq task %pK pending_cnt %d",
...
abort:
	cam_workq_put_task(task);
	CAM_INFO(CAM_ISP, "task aborted and queued back to pool");
	return rc;
}
	...
	int flags, void (*func)(struct work_struct *w))
{
	...
	if (!*workq) {
		cam_workq = kzalloc(sizeof(struct cam_core_workq),
			GFP_KERNEL);
		if (cam_workq == NULL)
			return -ENOMEM;
		...
		CAM_DBG(CAM_ISP, "create workque cam_workq-%s", name);
		...
		CAM_DBG(CAM_ISP, "LOCK_DBG workq %s lock %pK",
		...
		cam_workq->task.pool = kcalloc(cam_workq->task.num_task,
				sizeof(struct cam_workq_task), GFP_KERNEL);
		if (!cam_workq->task.pool) {
			CAM_WARN(CAM_ISP, "Insufficient memory %zu",
...
void cam_workq_destroy(struct cam_core_workq **cam_workq)
{
	...
	if (cam_workq && *cam_workq) {
		workq = *cam_workq;
		CAM_DBG(CAM_ISP, "destroy workque %s", workq->workq_name);
		WORKQ_ACQUIRE_LOCK(workq, flags);
