...
static struct ais_isp_mgr_data ais_ife_data = {
	.hw_type = AIS_ISP_MGR_HW_TYPE_IFE,
};

static struct ais_isp_mgr_data ais_tfe_data = {
	.hw_type = AIS_ISP_MGR_HW_TYPE_TFE,
};

static struct ais_ife_mgr g_ife_mgr;
...
#if 0 //[TODO]: add ad marco
	ais_vfe_discard_old_frame_done_event(vfe_idx, evt_data);
#else
	ais_tfe_discard_old_frame_done_event(vfe_idx, evt_data);
#endif
}
...
{
	int i;
	struct ais_isp_mgr_data *isp_mgr_data = p_ife_dev->isp_mgr_data;
	...
	for (i = 0; i < query_isp->num_dev; i++) {
		if (isp_mgr_data->hw_type == AIS_ISP_MGR_HW_TYPE_IFE)
			query_isp->dev_caps[i].hw_type = CAM_ISP_HW_IFE;
		else
			query_isp->dev_caps[i].hw_type = CAM_ISP_HW_MC_TFE;
...
static int ais_ife_partial_release(struct ais_ife_dev *p_ife_dev,
	struct ais_ife_rdi_deinit_args *rdi_deinit,
	uint32_t cmd_size)
{
	int rc = 0;
	int tmp;
	struct cam_hw_intf *p_grp_csid_drv;
	struct cam_hw_intf *p_grp_vfe_drv;
	struct ais_ife_rdi_deinit_args grp_rdi_deinit;
	struct ais_ife_stream_config *p_stream_cfg;
	struct ais_ife_dev *p_grp_ife_dev;
	int i, j;
	bool need_release = true;

	for (i = 0; i < p_ife_dev->num_grp_ife_dev; i++) {
		p_grp_ife_dev = p_ife_dev->p_grp_ife_dev[i];
		if (p_ife_dev == p_grp_ife_dev &&
			p_ife_dev->reserved_path_mask ==
				(1 << rdi_deinit->path)) {
			continue;
		}

		if (p_grp_ife_dev->reserved_path_mask) {
			need_release = false;
			break;
		}
	}
	...
	if (need_release) {
		for (i = 0; i < p_ife_dev->num_grp_ife_dev; i++) {
			p_grp_ife_dev = p_ife_dev->p_grp_ife_dev[i];
			p_stream_cfg = p_grp_ife_dev->p_stream_cfg;
			p_grp_csid_drv = p_grp_ife_dev->p_csid_drv;
			p_grp_vfe_drv = p_grp_ife_dev->p_vfe_drv;

			for (j = 0; j < p_stream_cfg->num_rdi; j++) {
				if (!(p_grp_ife_dev)) {
					CAM_ERR(CAM_ISP, "IFE %u not available",
						p_stream_cfg->hw_idx);
					continue;
				}

				if (!(p_grp_ife_dev->partial_reserved_path_mask &
					(1 << p_stream_cfg->rdi_cfg[j].path))) {
					CAM_DBG(CAM_ISP, "IFE %u RDI: %d"
						"is not partial reserved",
						p_stream_cfg->hw_idx, j);
					continue;
				}

				grp_rdi_deinit.path = p_stream_cfg->rdi_cfg[j].path;

				rc = p_grp_csid_drv->hw_ops.release(
					p_grp_csid_drv->hw_priv,
					&grp_rdi_deinit,
					sizeof(grp_rdi_deinit));

				tmp = p_grp_csid_drv->hw_ops.deinit(
					p_grp_csid_drv->hw_priv,
					&grp_rdi_deinit,
					sizeof(grp_rdi_deinit));
				if (!rc)
				...
				tmp = p_grp_vfe_drv->hw_ops.deinit(
					p_grp_vfe_drv->hw_priv,
					&grp_rdi_deinit,
					sizeof(grp_rdi_deinit));
				if (!rc)
					rc = tmp;

				p_grp_ife_dev->partial_reserved_path_mask &=
					~(1 << p_stream_cfg->rdi_cfg[j].path);

				CAM_DBG(CAM_ISP, "IFE %u RDI: %d partial released",
					p_grp_ife_dev->hw_idx, p_stream_cfg->rdi_cfg[j].path);
			}
		}
	}
...
static int ais_ife_partial_reserve(struct ais_ife_dev *p_ife_dev,
	...
	for (i = 0; i < p_ife_dev->num_grp_ife_dev; i++) {
		...
		for (j = 0; j < p_stream_cfg->num_rdi; j++) {
			...
			if (p_grp_ife_dev->partial_reserved_path_mask &
				(1 << p_stream_cfg->rdi_cfg[j].path)) {
				CAM_DBG(CAM_ISP, "IFE %u RDI: %d"
					" is already partial reserved",
					p_stream_cfg->hw_idx, j);
				continue;
			}

			rdi_deinit.path = p_stream_cfg->rdi_cfg[j].path;

			rc = p_grp_vfe_drv->hw_ops.init(
				p_grp_vfe_drv->hw_priv,
				&p_stream_cfg->rdi_cfg[j],
				sizeof(p_stream_cfg->rdi_cfg[j]));
			if (rc)
				goto fail_vfe_init;

			rc = p_grp_csid_drv->hw_ops.init(
				p_grp_csid_drv->hw_priv,
				&p_stream_cfg->rdi_cfg[j],
				sizeof(p_stream_cfg->rdi_cfg[j]));
			if (rc)
			...
			rc = p_grp_csid_drv->hw_ops.reserve(
				p_grp_csid_drv->hw_priv,
				&p_stream_cfg->rdi_cfg[j],
				sizeof(p_stream_cfg->rdi_cfg[j]));
			if (rc)
			...
			CAM_DBG(CAM_ISP, "IFE %u RDI: %d partial reserved",
				p_grp_ife_dev->hw_idx, p_stream_cfg->rdi_cfg[j].path);
		}
	}
...
fail_csid_reserve:
	p_grp_csid_drv->hw_ops.deinit(
		p_grp_csid_drv->hw_priv, &rdi_deinit, sizeof(rdi_deinit));
fail_csid_init:
	p_grp_vfe_drv->hw_ops.deinit(
		p_grp_vfe_drv->hw_priv, &rdi_deinit, sizeof(rdi_deinit));
fail_vfe_init:
	rdi_deinit.path = rdi_init->path;
	ais_ife_partial_release(
		p_ife_dev, &rdi_deinit, sizeof(rdi_deinit));
	return rc;
}
	...
	uint32_t cmd_size)
{
	...
	if (p_ife_dev->p_stream_cfg) {
		rc = ais_ife_partial_reserve(
			p_ife_dev, rdi_init,
			cmd_size);
		if (rc)
			goto fail_partial_reserve;
	} else {
	...
	p_ife_dev->reserved_path_mask |= (1 << rdi_init->path);
	CAM_DBG(CAM_ISP, "IFE %u RDI: %d full reserved",
...
fail_vfe_init:
fail_partial_reserve:
	if (p_ife_dev->p_stream_cfg) {
		// Partial reserve fail has been handled in ais_ife_partial_reserve
	}
	...
	uint32_t cmd_size)
{
	...
	rc = vfe_drv->hw_ops.release(
		vfe_drv->hw_priv, rdi_deinit, cmd_size);


	if (p_ife_dev->p_stream_cfg) {
		rc = ais_ife_partial_release(
			p_ife_dev, rdi_deinit, cmd_size);
	} else {
		tmp = csid_drv->hw_ops.release(
	...
	p_ife_dev->reserved_path_mask &= ~(1 << rdi_deinit->path);

	return rc;
}

static void ais_ife_dump_stream_group_cfg(struct ais_ife_stream_group_config_args *p_stream_grp_cfg)
{
	uint32_t i, j;
	size_t len = 0;
	uint8_t log_buf[512];
	struct ais_ife_rdi_init_args *p_rdi_cfg;

	CAM_INFO(CAM_ISP, "Stream group config:");
	for (i = 0; i < p_stream_grp_cfg->num_hw; i++) {
		len = 0;
		for (j = 0; j < p_stream_grp_cfg->stream_cfg[i].num_rdi; j++) {
			p_rdi_cfg = &p_stream_grp_cfg->stream_cfg[i].rdi_cfg[j];
			len += scnprintf(log_buf + len, sizeof(log_buf) - len,
					" rdi_cfg[%u]: { csiphy: %u vc:%u dt:%u dt_id: %u}",
					p_rdi_cfg->path,
					p_rdi_cfg->csi_cfg.csiphy_id,
					p_rdi_cfg->csi_cfg.vc,
					p_rdi_cfg->csi_cfg.dt,
					p_rdi_cfg->csi_cfg.dt_id);
		}

		CAM_INFO(CAM_ISP, "  hw_idx: %d csiphy_id: %d %s",
			p_stream_grp_cfg->stream_cfg[i].hw_idx,
			p_stream_grp_cfg->stream_cfg[i].csiphy_id,
			log_buf);
	}
}

static void ais_ife_fill_group_info(struct ais_ife_stream_group_config_args *p_stream_grp_cfg)
{
	// int rc;
	struct ais_ife_stream_config *p_stream_cfg, *p_grp_stream_cfg;
	struct ais_ife_dev *p_ife_dev, *p_grp_ife_dev;
	int i, j;
	size_t len = 0;
	uint8_t log_buf[100];


	for (i = 0; i < g_ife_mgr.stream_grp_cfg.num_hw; i++) {
		p_stream_cfg = &g_ife_mgr.stream_grp_cfg.stream_cfg[i];
		p_ife_dev = g_ife_mgr.p_ife_dev[p_stream_cfg->hw_idx];
		len = 0;

		if (p_ife_dev == NULL)
			continue;

		p_ife_dev->csiphy_id = p_stream_cfg->csiphy_id;
		p_ife_dev->p_stream_cfg = p_stream_cfg;
		p_ife_dev->num_grp_ife_dev = 0;

		for (j = 0; j < g_ife_mgr.stream_grp_cfg.num_hw; j++) {
			p_grp_stream_cfg = &g_ife_mgr.stream_grp_cfg.stream_cfg[j];
			p_grp_ife_dev = g_ife_mgr.p_ife_dev[p_grp_stream_cfg->hw_idx];

			if (p_grp_ife_dev == NULL)
				continue;

			// Add self into grouped device to avoid special handling
			if (p_grp_stream_cfg->csiphy_id == p_stream_cfg->csiphy_id) {
				p_ife_dev->p_grp_ife_dev[p_ife_dev->num_grp_ife_dev++] = p_grp_ife_dev;

				len += scnprintf(log_buf + len, sizeof(log_buf) - len,
					"%u ", p_grp_ife_dev->hw_idx);
			}
		}
		CAM_INFO(CAM_ISP, "IFE %u grouped dev: %s", p_ife_dev->hw_idx, log_buf);
	}
}
...
static int ais_ife_driver_cmd(struct ais_ife_dev *p_ife_dev, void *arg)
{
	...
	case AIS_IFE_DIAG_INFO: {
		...
		break;
	case AIS_IFE_UPDATE_STREAM_GROUP_CONFIG: {

		if (cmd->size != sizeof(g_ife_mgr.stream_grp_cfg)) {
			rc = -EINVAL;
		} else if (copy_from_user(&g_ife_mgr.stream_grp_cfg,
				u64_to_user_ptr(cmd->handle),
				cmd->size)) {
			rc = -EFAULT;
		} else {
			// Dump group cfg and fill group info into device
			ais_ife_dump_stream_group_cfg(&g_ife_mgr.stream_grp_cfg);
			ais_ife_fill_group_info(&g_ife_mgr.stream_grp_cfg);
		}
	}
		break;
	default:
...
static void ais_ife_dev_iommu_fault_handler(struct cam_smmu_pf_info *pf_smmu_info)
{
	...
	p_ife_dev = (struct ais_ife_dev *)pf_smmu_info->token;

	CAM_ERR(CAM_ISP, "IFE%d Pagefault at iova 0x%x %u",
			p_ife_dev->hw_idx, pf_smmu_info->iova, pf_smmu_info->domain->type);
	...
	struct device *master_dev, void *data)
{
	...
	/* clean up resources */
	g_ife_mgr.p_ife_dev[p_ife_dev->hw_idx] = NULL;
	...
	struct device *master_dev, void *data)
{
	...
	p_ife_dev->partial_reserved_path_mask = 0;
	p_ife_dev->reserved_path_mask         = 0;
	p_ife_dev->num_grp_ife_dev            = 0;
	p_ife_dev->p_stream_cfg               = NULL;
	p_ife_dev->isp_mgr_data               = (struct ais_isp_mgr_data  *)match_dev->data;
	memset(p_ife_dev->p_grp_ife_dev, 0, sizeof(p_ife_dev->p_grp_ife_dev));
	 ...
	 *  deinit support
	 */
	rc = cam_smmu_get_handle("tfe", &p_ife_dev->iommu_hdl);
	if (rc) {
	...
	if (p_ife_dev->isp_mgr_data->hw_type == AIS_ISP_MGR_HW_TYPE_IFE) {
		...
		rc = ais_tfe_csid_hw_init(&p_ife_dev->p_csid_drv, &hw_init);
		if (rc) {
			CAM_ERR(CAM_ISP, "TFE%d no CSID dev rc:%d", p_ife_dev->hw_idx, rc);
			goto secure_fail;
		}

		rc = ais_tfe_hw_init(&p_ife_dev->p_vfe_drv, &hw_init,
				p_ife_dev->p_csid_drv);
		if (rc) {
			CAM_ERR(CAM_ISP, "TFE%d no VFE dev rc:%d", p_ife_dev->hw_idx, rc);
			goto secure_fail;
		}
	}
	...
	g_ife_mgr.p_ife_dev[p_ife_dev->hw_idx] = p_ife_dev;
...
static int ais_ife_init_subdev_params(struct ais_ife_dev *p_ife_dev)
{
	...
	p_ife_dev->cam_sd.ops =
		&ais_ife_subdev_ops;
	strscpy(p_ife_dev->device_name, AIS_IFE_DEV_NAME,
...
#else
 static void ais_ife_dev_remove(struct platform_device *pdev)
#endif
{
...
{
	memset(&g_ife_mgr, 0, sizeof(g_ife_mgr));
