...
#ifndef _AIS_TFE_CORE_H_
#define _AIS_TFE_CORE_H_
...
struct ais_vfe_hw_info {
	...
	struct ais_tfe_bus_ver2_hw_info  *bus_hw_info;

	uint32_t                          bus_rd_version;
	void                             *bus_rd_hw_info;

	uint32_t                          top_version;
	struct ais_tfe_top_ver2_hw_info  *top_hw_info;
...
struct ais_vfe_hw_core_info {
	...
	uint32_t                            max_rdis;
	struct ais_tfe_rdi_output           rdi_out[AIS_IFE_PATH_MAX];
...
int ais_tfe_init_hw(void *device_priv,
	void *init_hw_args, uint32_t arg_size);
int ais_tfe_deinit_hw(void *hw_priv,
	void *deinit_hw_args, uint32_t arg_size);
int ais_tfe_force_reset(void *device_priv,
	void *reset_core_args, uint32_t arg_size);
int ais_tfe_reserve(void *device_priv,
	void *reserve_args, uint32_t arg_size);
int ais_tfe_release(void *device_priv,
	void *reserve_args, uint32_t arg_size);
int ais_tfe_start(void *device_priv,
	void *start_args, uint32_t arg_size);
int ais_tfe_stop(void *device_priv,
	void *stop_args, uint32_t arg_size);
int ais_tfe_read(void *device_priv,
	void *read_args, uint32_t arg_size);
int ais_tfe_write(void *device_priv,
	void *write_args, uint32_t arg_size);
int ais_tfe_process_cmd(void *device_priv, uint32_t cmd_type,
