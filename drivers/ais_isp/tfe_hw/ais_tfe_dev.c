// SPDX-License-Identifier: GPL-2.0-only
/*
 * Copyright (c) 2017-2020, The Linux Foundation. All rights reserved.
 * Copyright (c) Qualcomm Technologies, Inc. and/or its subsidiaries.
 */


#include <linux/slab.h>
#include <linux/mod_devicetable.h>
#include <linux/of_device.h>
#include <linux/component.h>
#include "ais_tfe_dev.h"
#include "ais_tfe_core.h"
#include "ais_tfe_soc.h"
#include "cam_debug_util.h"

static struct cam_hw_intf *ais_tfe_hw_list[AIS_TFE_HW_NUM_MAX] = {
	NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL};

static char tfe_dev_name[8];

static int ais_tfe_dev_component_bind(struct device *dev,
	struct device *master_dev, void *data)
{
	struct cam_hw_info                *vfe_hw = NULL;
	struct cam_hw_intf                *vfe_hw_intf = NULL;
	const struct of_device_id         *match_dev = NULL;
	struct ais_vfe_hw_core_info       *core_info = NULL;
	struct ais_vfe_hw_info            *hw_info = NULL;
	int                                rc = 0;
	struct platform_device *pdev = to_platform_device(dev);

	CAM_DBG(CAM_ISP, "TFE component bind called");

	vfe_hw_intf = kzalloc(sizeof(struct cam_hw_intf), GFP_KERNEL);
	if (!vfe_hw_intf) {
		rc = -ENOMEM;
		goto end;
	}

	of_property_read_u32(pdev->dev.of_node,
		"cell-index", &vfe_hw_intf->hw_idx);

	vfe_hw = kzalloc(sizeof(struct cam_hw_info), GFP_KERNEL);
	if (!vfe_hw) {
		rc = -ENOMEM;
		goto free_vfe_hw_intf;
	}

	memset(tfe_dev_name, 0, sizeof(tfe_dev_name));
	snprintf(tfe_dev_name, sizeof(tfe_dev_name),
		"tfe%1u", vfe_hw_intf->hw_idx);

	vfe_hw->soc_info.pdev = pdev;
	vfe_hw->soc_info.dev = &pdev->dev;
	vfe_hw->soc_info.dev_name = tfe_dev_name;
	vfe_hw_intf->hw_priv = vfe_hw;
	vfe_hw_intf->hw_ops.get_hw_caps = ais_tfe_get_hw_caps;
	vfe_hw_intf->hw_ops.init = ais_tfe_init_hw;
	vfe_hw_intf->hw_ops.deinit = ais_tfe_deinit_hw;
	vfe_hw_intf->hw_ops.reset = ais_tfe_force_reset;
	vfe_hw_intf->hw_ops.reserve = ais_tfe_reserve;
	vfe_hw_intf->hw_ops.release = ais_tfe_release;
	vfe_hw_intf->hw_ops.start = ais_tfe_start;
	vfe_hw_intf->hw_ops.stop = ais_tfe_stop;
	vfe_hw_intf->hw_ops.read = ais_tfe_read;
	vfe_hw_intf->hw_ops.write = ais_tfe_write;
	vfe_hw_intf->hw_ops.process_cmd = ais_tfe_process_cmd;
	vfe_hw_intf->hw_type = AIS_ISP_HW_TYPE_TFE;

	CAM_DBG(CAM_ISP, "Component bind called for TFE%d", vfe_hw_intf->hw_idx);

	platform_set_drvdata(pdev, vfe_hw_intf);

	vfe_hw->core_info = kzalloc(sizeof(struct ais_vfe_hw_core_info),
		GFP_KERNEL);
	if (!vfe_hw->core_info) {
		CAM_DBG(CAM_ISP, "Failed to alloc for core");
		rc = -ENOMEM;
		goto free_vfe_hw;
	}
	core_info = (struct ais_vfe_hw_core_info *)vfe_hw->core_info;

	match_dev = of_match_device(pdev->dev.driver->of_match_table,
		&pdev->dev);
	if (!match_dev) {
		CAM_ERR(CAM_ISP, "Of_match Failed");
		rc = -EINVAL;
		goto free_core_info;
	}
	hw_info = (struct ais_vfe_hw_info *)match_dev->data;
	core_info->vfe_hw_info = hw_info;

	rc = ais_tfe_init_soc_resources(&vfe_hw->soc_info, ais_tfe_irq,
		vfe_hw);
	if (rc < 0) {
		CAM_ERR(CAM_ISP, "Failed to init soc rc=%d", rc);
		goto free_core_info;
	}

	rc = ais_tfe_core_init(core_info, &vfe_hw->soc_info,
		vfe_hw_intf, hw_info);
	if (rc < 0) {
		CAM_ERR(CAM_ISP, "Failed to init core rc=%d", rc);
		goto deinit_soc;
	}

	vfe_hw->hw_state = CAM_HW_STATE_POWER_DOWN;
	mutex_init(&vfe_hw->hw_mutex);
	spin_lock_init(&vfe_hw->hw_lock);
	init_completion(&vfe_hw->hw_complete);

	if (vfe_hw_intf->hw_idx < AIS_TFE_HW_NUM_MAX)
		ais_tfe_hw_list[vfe_hw_intf->hw_idx] = vfe_hw_intf;

	// ais_tfe_init_hw(vfe_hw, NULL, 0);
	// ais_tfe_deinit_hw(vfe_hw, NULL, 0);

	CAM_DBG(CAM_ISP, "TFE%d component bound successfully", vfe_hw_intf->hw_idx);

	return rc;

deinit_soc:
	if (ais_tfe_deinit_soc_resources(&vfe_hw->soc_info))
		CAM_ERR(CAM_ISP, "Failed to deinit soc");
free_core_info:
	kfree(vfe_hw->core_info);
free_vfe_hw:
	kfree(vfe_hw);
free_vfe_hw_intf:
	kfree(vfe_hw_intf);
end:
	return rc;
}

static void ais_tfe_dev_component_unbind(struct device *dev,
	struct device *master_dev, void *data)
{
	struct cam_hw_info                *vfe_hw = NULL;
	struct cam_hw_intf                *vfe_hw_intf = NULL;
	struct ais_vfe_hw_core_info       *core_info = NULL;
	int                                rc = 0;
	struct platform_device *pdev = to_platform_device(dev);

	vfe_hw_intf = platform_get_drvdata(pdev);
	if (!vfe_hw_intf) {
		CAM_ERR(CAM_ISP, "Error! No data in pdev");
		return;
	}

	CAM_DBG(CAM_ISP, "type %d index %d",
		vfe_hw_intf->hw_type, vfe_hw_intf->hw_idx);

	if (vfe_hw_intf->hw_idx < AIS_TFE_HW_NUM_MAX)
		ais_tfe_hw_list[vfe_hw_intf->hw_idx] = NULL;

	vfe_hw = vfe_hw_intf->hw_priv;
	if (!vfe_hw) {
		CAM_ERR(CAM_ISP, "Error! HW data is NULL");
		goto free_vfe_hw_intf;
	}

	core_info = (struct ais_vfe_hw_core_info *)vfe_hw->core_info;
	if (!core_info) {
		CAM_ERR(CAM_ISP, "Error! core data NULL");
		goto deinit_soc;
	}

	rc = ais_tfe_core_deinit(core_info, core_info->vfe_hw_info);
	if (rc < 0)
		CAM_ERR(CAM_ISP, "Failed to deinit core rc=%d", rc);

	kfree(vfe_hw->core_info);

deinit_soc:
	rc = ais_tfe_deinit_soc_resources(&vfe_hw->soc_info);
	if (rc < 0)
		CAM_ERR(CAM_ISP, "Failed to deinit soc rc=%d", rc);

	mutex_destroy(&vfe_hw->hw_mutex);
	kfree(vfe_hw);

	CAM_DBG(CAM_ISP, "TFE%d remove successful", vfe_hw_intf->hw_idx);

free_vfe_hw_intf:
	kfree(vfe_hw_intf);

}

int ais_tfe_hw_init(struct cam_hw_intf **vfe_hw,
	struct ais_isp_hw_init_args *init,
	struct cam_hw_intf *csid_hw)
{
	int rc = 0;

	if (ais_tfe_hw_list[init->hw_idx]) {
		struct cam_hw_info          *vfe_hw_info = NULL;
		struct ais_vfe_hw_core_info *core_info = NULL;
		//struct cam_hw_info  *csid_hw_info = NULL;
		//struct ais_tfe_csid_ver2_hw *csid_core = NULL;

		vfe_hw_info = ais_tfe_hw_list[init->hw_idx]->hw_priv;
		core_info =
			(struct ais_vfe_hw_core_info *)vfe_hw_info->core_info;

		core_info->csid_hw = csid_hw;

		core_info->event_cb = init->event_cb;
		core_info->event_cb_priv = init->event_cb_priv;
		core_info->iommu_hdl = init->iommu_hdl;
		core_info->iommu_hdl_secure = init->iommu_hdl_secure;

		/* add csid irq event callback */
		//csid_hw_info = (struct cam_hw_info*)csid_hw->hw_priv;
		//csid_core = (struct ais_tfe_csid_ver2_hw*)(csid_hw_info->core_info);
		//csid_core->vfe_event_cb = ais_tfe_csid_irq_event_callback;
		//csid_core->vfe_hw_info = (void*)vfe_hw_info;

		*vfe_hw = ais_tfe_hw_list[init->hw_idx];
		rc = 0;
	} else {
		*vfe_hw = NULL;
		rc = -ENODEV;
	}
	return rc;
}

static const struct component_ops ais_tfe_dev_component_ops = {
	.bind = ais_tfe_dev_component_bind,
	.unbind = ais_tfe_dev_component_unbind,
};

int ais_tfe_probe(struct platform_device *pdev)
{
	int rc = 0;

	CAM_DBG(CAM_ISP, "Adding TFE dev component");

	rc = component_add(&pdev->dev, &ais_tfe_dev_component_ops);
	if (rc)
		CAM_ERR(CAM_ISP, "failed to add component rc: %d", rc);

	return rc;
}

#if KERNEL_VERSION(6, 10, 0) > LINUX_VERSION_CODE
int ais_tfe_remove(struct platform_device *pdev)
#else
void ais_tfe_remove(struct platform_device *pdev)
#endif
{
	component_del(&pdev->dev, &ais_tfe_dev_component_ops);

#if KERNEL_VERSION(6, 10, 0) > LINUX_VERSION_CODE
	return 0;
#endif
}

void ais_tfe_discard_old_frame_done_event(uint8_t tfe_idx,
					struct ais_ife_event_data *evt_data)
{
	struct cam_hw_info *vfe_info = NULL;
	struct ais_vfe_hw_core_info       *core_info = NULL;

	if (tfe_idx >= AIS_TFE_HW_NUM_MAX || evt_data == NULL)
		return;

	vfe_info = ais_tfe_hw_list[tfe_idx]->hw_priv;
	if (vfe_info == NULL || vfe_info->core_info == NULL)
		return;
	core_info = (struct ais_vfe_hw_core_info *)vfe_info->core_info;

	ais_tfe_ife_discard_old_frame_done_event(core_info, evt_data);
}
