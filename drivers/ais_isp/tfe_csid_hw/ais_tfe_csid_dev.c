...
static struct cam_hw_intf *ais_tfe_csid_hw_list[AIS_TFE_CSID_HW_RES_MAX] = {
	NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL};

static char csid_dev_name[8];

static int ais_tfe_csid_dev_component_bind(struct device *dev,
	struct device *master_dev, void *data)
{
	...
	csid_hw_intf->hw_type = AIS_ISP_HW_TYPE_TFE_CSID;
	...
	rc = ais_tfe_csid_hw_probe_init(csid_hw_intf, csid_dev_idx);
	if (rc)
		goto free_dev;
	...
	if (csid_hw_intf->hw_idx < AIS_TFE_CSID_HW_RES_MAX)
		ais_tfe_csid_hw_list[csid_hw_intf->hw_idx] = csid_hw_intf;
	else
		goto free_dev;
...
static void ais_tfe_csid_dev_component_unbind(struct device *dev,
	struct device *master_dev, void *data)
{
	struct ais_ife_csid_hw         *csid_dev = NULL;
	struct cam_hw_intf             *csid_hw_intf;
	...
	ais_tfe_csid_hw_deinit(csid_dev);
...
int ais_tfe_csid_hw_init(struct cam_hw_intf **tfe_csid_hw,
		struct ais_isp_hw_init_args *init)
{
	int rc = 0;

	if (ais_tfe_csid_hw_list[init->hw_idx]) {
		struct cam_hw_info *csid_hw_info = NULL;
		struct ais_ife_csid_hw *core_info = NULL;

		csid_hw_info = ais_tfe_csid_hw_list[init->hw_idx]->hw_priv;
		...
		*tfe_csid_hw = ais_tfe_csid_hw_list[init->hw_idx];
	} else {
		*tfe_csid_hw = NULL;
		rc = -1;
	}
...
static const struct component_ops ais_tfe_csid_dev_component_ops = {
	.bind = ais_tfe_csid_dev_component_bind,
	.unbind = ais_tfe_csid_dev_component_unbind,
};

int ais_tfe_csid_probe(struct platform_device *pdev)
{
	int rc = 0;

	CAM_DBG(CAM_ISP, "Adding CSID dev component");
	rc = component_add(&pdev->dev, &ais_tfe_csid_dev_component_ops);
	if (rc)
...
int ais_tfe_csid_remove(struct platform_device *pdev)
#else
void ais_tfe_csid_remove(struct platform_device *pdev)
#endif
{
	component_del(&pdev->dev, &ais_tfe_csid_dev_component_ops);
#if KERNEL_VERSION(6, 10, 0) > LINUX_VERSION_CODE
	return 0;
#endif
}
