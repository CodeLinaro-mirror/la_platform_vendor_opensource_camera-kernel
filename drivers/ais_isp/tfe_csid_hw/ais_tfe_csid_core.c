...
/* TPG VC/DT values */
#define AIS_TFE_CSID_TPG_VC_VAL                        0xA
#define AIS_TFE_CSID_TPG_DT_VAL                        0x2B

/* Timeout values in usec */
#define AIS_TFE_CSID_TIMEOUT_SLEEP_US                  1000
#define AIS_TFE_CSID_TIMEOUT_ALL_US                    100000
 ...
 */
#define AIS_TFE_CSID_QTIMER_MUL_FACTOR                 10000
#define AIS_TFE_CSID_QTIMER_DIV_FACTOR                 192

/* Max number of sof irq's triggered in case of SOF freeze */
#define AIS_CSID_IRQ_SOF_DEBUG_CNT_MAX 12

/* Max CSI Rx irq error count threshold value */
#define AIS_TFE_CSID_MAX_IRQ_ERROR_COUNT               5

static int ais_tfe_csid_global_reset(struct ais_ife_csid_hw *csid_hw)
{
	...
	rc = cam_common_read_poll_timeout(soc_info->reg_map[0].mem_base +
			...
			AIS_TFE_CSID_TIMEOUT_SLEEP_US, AIS_TFE_CSID_TIMEOUT_ALL_US,
			0x1, 0x1, &status);
	if (rc < 0) {
	...
	if (rc < 0) {
		CAM_ERR(CAM_ISP, "CSID:%d csid sw reset fail rc = %d",
				csid_hw->hw_intf->hw_idx, rc);
		rc = -ETIMEDOUT;
	} else
		rc = 0;
	...
	for (i = 0 ; i < AIS_TFE_CSID_RDI_MAX; i++) {
...
static int ais_tfe_csid_path_reset(struct ais_ife_csid_hw *csid_hw,
		enum ais_ife_output_path_id  reset_path)
{
	...
	if (reset_path >= AIS_TFE_CSID_RDI_MAX) {
...
static int ais_tfe_csid_enable_hw(struct ais_ife_csid_hw  *csid_hw)
{
	int rc = 0;
	const struct ais_ife_csid_reg_offset      *csid_reg;
	struct cam_hw_soc_info              *soc_info;
	...
	rc = cam_soc_util_get_clk_level(&csid_hw->hw_info->soc_info,
			csid_hw->clk_rate,
			csid_hw->hw_info->soc_info.src_clk_idx, &clk_lvl);
	CAM_DBG(CAM_ISP, "CSID clock lvl %u", clk_lvl);

	rc = ais_tfe_csid_enable_soc_resources(soc_info, clk_lvl);
	if (rc) {
	...
	/* Reset CSID top */
	rc = ais_tfe_csid_global_reset(csid_hw);
	if (rc)
		goto disable_soc;
...
disable_soc:
	ais_tfe_csid_disable_soc_resources(soc_info);
...
static int ais_tfe_csid_disable_hw(struct ais_ife_csid_hw *csid_hw)
{
	int rc = -EINVAL;
	uint32_t i;
	struct cam_hw_soc_info                   *soc_info;
	...
	ais_tfe_csid_global_reset(csid_hw);
	...
	rc = ais_tfe_csid_disable_soc_resources(soc_info);
	if (rc)
...
static int ais_tfe_csid_enable_csi2(
...
static int ais_tfe_csid_disable_csi2(struct ais_ife_csid_hw *csid_hw)
{
	int rc = 0;
	const struct ais_ife_csid_reg_offset *csid_reg;
	struct cam_hw_soc_info               *soc_info;
...
static void ais_tfe_csid_halt_csi2(
		struct ais_ife_csid_hw          *csid_hw)
{
	const struct ais_ife_csid_reg_offset      *csid_reg;
	struct cam_hw_soc_info                    *soc_info;
...
static int ais_tfe_csid_get_format_rdi(
...
static int ais_tfe_csid_config_rdi_path(
	struct ais_ife_csid_hw          *csid_hw,
	struct ais_ife_rdi_init_args         *res)
{
	...
	uint32_t val = 0, cfg0 = 0, id = 0;
	uint32_t format_measure_addr;
	uint32_t path_format = 0, plain_fmt = 0;

	csid_reg = csid_hw->csid_info->csid_reg;
	soc_info = &csid_hw->hw_info->soc_info;

	id = res->path;
	if (id >= AIS_TFE_CSID_RDI_MAX || id >= csid_reg->cmn_reg->num_rdis ||
		!csid_reg->rdi_reg[id]) {
	...
	rc = ais_tfe_csid_get_format_rdi(path_cfg->in_format,
			path_cfg->out_format, &path_format, &plain_fmt);
	...
	//[TODO] for ais: disable crop first
	...
	/* Program the IPP CFG0 for full TFE in case of RPP path enabled */
	...
	/* cfg0: RDI path config and enable*/
	...
	if (csid_reg->cmn_reg->format_measure_support &&
		(csid_hw->csid_debug & CSID_DEBUG_ENABLE_HBI_VBI_INFO)) {
		format_measure_addr =
			csid_reg->rdi_reg[id]->csid_rdi_format_measure_cfg0_addr;
		val = cam_io_r_mb(soc_info->reg_map[0].mem_base +
			format_measure_addr);
...
static int ais_tfe_csid_deinit_rdi_path(
	struct ais_ife_csid_hw          *csid_hw,
...
static int ais_tfe_csid_enable_rdi_path(
	struct ais_ife_csid_hw          *csid_hw,
	struct ais_ife_rdi_start_args   *start_cmd)
{
	...
	if (start_cmd->path >= AIS_TFE_CSID_RDI_MAX) {
...
static int ais_tfe_csid_disable_rdi_path(
	struct ais_ife_csid_hw          *csid_hw,
	...
	enum ais_ife_csid_halt_cmd       stop_cmd)
{
	...
	if (stop_args->path >= AIS_TFE_CSID_RDI_MAX) {
...
static int ais_tfe_csid_poll_stop_status(
	struct ais_ife_csid_hw          *csid_hw,
	uint32_t                         res_mask)
{
	int rc = 0;
	...
	for (; res_id < AIS_TFE_CSID_RDI_MAX; res_id++, res_mask >>= 1) {
		if ((res_mask & 0x1) == 0)
			continue;
		val = 0;
		...
		rc = readl_poll_timeout(soc_info->reg_map[0].mem_base +
			...
			AIS_TFE_CSID_TIMEOUT_SLEEP_US,
			AIS_TFE_CSID_TIMEOUT_ALL_US);
		if (rc < 0) {
...
static int ais_tfe_csid_get_time_stamp(
	struct ais_ife_csid_hw   *csid_hw,
	void *cmd_args)
{
	...
	if (id >= AIS_TFE_CSID_RDI_MAX || !csid_reg->rdi_reg[id]) {
	...
	p_timestamp->ts->cur_sof_ts = mul_u64_u32_div(time_64,
		AIS_TFE_CSID_QTIMER_MUL_FACTOR,
		AIS_TFE_CSID_QTIMER_DIV_FACTOR);
...
static int ais_tfe_csid_set_csid_debug(struct ais_ife_csid_hw   *csid_hw,
	void *cmd_args)
{
	uint32_t  *csid_debug;
...
static int ais_tfe_csid_get_hw_caps(void *hw_priv,
...
static int ais_tfe_csid_force_reset(void *hw_priv,
	void *reset_args, uint32_t arg_size)
{
	...
	if (csid_hw_info->open_count) {
		...
		rc = ais_tfe_csid_disable_hw(csid_hw);
...
static int ais_tfe_csid_reset_retain_sw_reg(
	struct ais_ife_csid_hw *csid_hw)
{
	int rc = 0;
	uint32_t status;
...
static int ais_tfe_csid_reserve(void *hw_priv,
	void *reserve_args, uint32_t arg_size)
{
	...
	mutex_lock(&csid_hw->hw_info->hw_mutex);
	if (rdi_cfg->path >= AIS_TFE_CSID_RDI_MAX) {
	...
	rc = ais_tfe_csid_enable_csi2(csid_hw, &rdi_cfg->csi_cfg);
	if (rc)
		goto end;

	if (csid_hw->device_enabled == 0) {
		rc = ais_tfe_csid_reset_retain_sw_reg(csid_hw);
		if (rc < 0) {
	...
	rc = ais_tfe_csid_config_rdi_path(csid_hw, rdi_cfg);
	if (rc)
		goto disable_csi2;

disable_csi2:
	if (rc)
		ais_tfe_csid_disable_csi2(csid_hw);
end:
	mutex_unlock(&csid_hw->hw_info->hw_mutex);
...
static int ais_tfe_csid_release(void *hw_priv,
	void *release_args, uint32_t arg_size)
{
	...
	mutex_lock(&csid_hw->hw_info->hw_mutex);
	if (rdi_deinit->path >= AIS_TFE_CSID_RDI_MAX) {
	...
	rc = ais_tfe_csid_deinit_rdi_path(csid_hw, rdi_deinit->path);

	CAM_DBG(CAM_ISP, "De-Init tfe_csid");
	rc |= ais_tfe_csid_disable_csi2(csid_hw);
...
static int ais_tfe_csid_init_hw(void *hw_priv,
	void *init_args, uint32_t arg_size)
{
	...
	rc = ais_tfe_csid_enable_hw(csid_hw);
...
static int ais_tfe_csid_deinit_hw(void *hw_priv,
	void *deinit_args, uint32_t arg_size)
{
	...
	ais_tfe_csid_path_reset(csid_hw, deinit->path);
...
static int ais_tfe_csid_start(void *hw_priv, void *start_args,
			uint32_t arg_size)
{
	int rc = 0;
	struct ais_ife_csid_hw                 *csid_hw;
	...
	rc = ais_tfe_csid_enable_rdi_path(csid_hw, start_cmd);
end:
	return rc;
}

static int ais_tfe_csid_stop(void *hw_priv,
	void *stop_args, uint32_t arg_size)
{
	...
	rc = ais_tfe_csid_disable_rdi_path(csid_hw, stop_cmd,
			AIS_CSID_HALT_IMMEDIATELY);

	if (res_mask)
		rc = ais_tfe_csid_poll_stop_status(csid_hw, res_mask);
...
static int ais_tfe_csid_read(void *hw_priv,
...
static int ais_tfe_csid_write(void *hw_priv,
...
static int ais_tfe_csid_sof_irq_debug(
...
static int ais_tfe_csid_set_csid_clock(
	struct ais_ife_csid_hw *csid_hw, void *cmd_args)
{
...
static int ais_tfe_csid_dump_hw(
	struct ais_ife_csid_hw *csid_hw, void *cmd_args)
{
	struct cam_hw_soc_info                         *soc_info;
...
static int ais_tfe_csid_get_total_pkts(
	struct ais_ife_csid_hw *csid_hw, void *cmd_args)
{
...
static int ais_tfe_csid_print_overflow_info(
	struct ais_ife_csid_hw *csid_hw, void *cmd_args)
{
	int rc = 0;
	uint32_t byteCntr[2] = {};
...
static int ais_tfe_csid_process_cmd(void *hw_priv,
	uint32_t cmd_type, void *cmd_args, uint32_t arg_size)
{
	...
	case AIS_IFE_CSID_CMD_GET_TIME_STAMP:
		rc = ais_tfe_csid_get_time_stamp(csid_hw, cmd_args);
		break;
	case AIS_IFE_CSID_SET_CSID_DEBUG:
		rc = ais_tfe_csid_set_csid_debug(csid_hw, cmd_args);
		break;
	case AIS_IFE_CSID_SOF_IRQ_DEBUG:
		rc = ais_tfe_csid_sof_irq_debug(csid_hw, cmd_args);
		break;
	case AIS_ISP_HW_CMD_CSID_CLOCK_UPDATE:
		rc = ais_tfe_csid_set_csid_clock(csid_hw, cmd_args);
		break;
	case AIS_ISP_HW_CMD_DUMP_HW:
		rc = ais_tfe_csid_dump_hw(csid_hw, cmd_args);
		break;
	case AIS_IFE_CSID_CMD_DIAG_INFO:
		rc = ais_tfe_csid_get_total_pkts(csid_hw, cmd_args);
		break;
	case AIS_IFE_CSID_CMD_OVERFLOW_INFO:
		rc = ais_tfe_csid_print_overflow_info(csid_hw, cmd_args);
		break;
	default:
...
static irqreturn_t ais_tfe_csid_irq(int irq_num, void *data)
{
	struct ais_ife_csid_hw                         *csid_hw;
	struct cam_hw_soc_info                         *soc_info;
	const struct ais_ife_csid_reg_offset           *csid_reg;
	...
	/*[TODO] for ais: software reset to check the bit*/
	/* Software register reset complete*/
	if (irq_status[CSID_IRQ_STATUS_TOP] & 0x1)
	...
	if (csid_hw->error_irq_count >
		AIS_TFE_CSID_MAX_IRQ_ERROR_COUNT) {
		fatal_err_detected = true;
		csid_hw->error_irq_count = 0;
	} else if (warn_cnt) {
		uint64_t timestamp;
...
handle_fatal_error:
	...
	if (fatal_err_detected) {
		...
		CAM_INFO(CAM_ISP,
			...
			csid_hw->hw_intf->hw_idx, csid_hw->csi2_cfg_cnt,
			irq_status[CSID_IRQ_STATUS_RX]);
		ais_tfe_csid_halt_csi2(csid_hw);
		ais_csid_dispatch_irq(csid_hw,
	...
	if ((csid_hw->csid_debug & CSID_DEBUG_ENABLE_SHORT_PKT_CAPTURE) &&
		...
		val = (1 << csi2_reg->csi2_rx_long_pkt_hdr_rst_stb_shift);
		cam_io_w_mb(val, soc_info->reg_map[0].mem_base +
				csi2_reg->csi2_rx_short_pkt_hdr_rst_stb_shift);
	}
	...
	if (csid_hw->irq_debug_cnt >= AIS_CSID_IRQ_SOF_DEBUG_CNT_MAX) {
		ais_tfe_csid_sof_irq_debug(csid_hw, &sof_irq_debug_en);
		csid_hw->irq_debug_cnt = 0;
	}
...
int ais_tfe_csid_hw_probe_init(struct cam_hw_intf  *csid_hw_intf,
	uint32_t csid_idx)
{
	int rc = -EINVAL;
	uint32_t i;
	...
	if (csid_idx >= AIS_TFE_CSID_HW_RES_MAX) {
	...
	rc = ais_tfe_csid_init_soc_resources(&ife_csid_hw->hw_info->soc_info,
			ais_tfe_csid_irq, ife_csid_hw);
	if (rc < 0) {
		CAM_ERR(CAM_ISP, "CSID:%d Failed to init_soc", csid_idx);
		goto err;
	}

	rc = cam_soc_util_get_clk_level(&ife_csid_hw->hw_info->soc_info,
			ife_csid_hw->clk_rate,
			ife_csid_hw->hw_info->soc_info.src_clk_idx, &clk_lvl);
	CAM_DBG(CAM_ISP, "CSID clock lvl %u", clk_lvl);

	ife_csid_hw->hw_intf->hw_ops.get_hw_caps = ais_tfe_csid_get_hw_caps;
	ife_csid_hw->hw_intf->hw_ops.init        = ais_tfe_csid_init_hw;
	ife_csid_hw->hw_intf->hw_ops.deinit      = ais_tfe_csid_deinit_hw;
	ife_csid_hw->hw_intf->hw_ops.reset       = ais_tfe_csid_force_reset;
	ife_csid_hw->hw_intf->hw_ops.reserve     = ais_tfe_csid_reserve;
	ife_csid_hw->hw_intf->hw_ops.release     = ais_tfe_csid_release;
	ife_csid_hw->hw_intf->hw_ops.start       = ais_tfe_csid_start;
	ife_csid_hw->hw_intf->hw_ops.stop        = ais_tfe_csid_stop;
	ife_csid_hw->hw_intf->hw_ops.read        = ais_tfe_csid_read;
	ife_csid_hw->hw_intf->hw_ops.write       = ais_tfe_csid_write;
	ife_csid_hw->hw_intf->hw_ops.process_cmd = ais_tfe_csid_process_cmd;
...
err_deinit_soc:
	ais_tfe_csid_deinit_soc_resources(&ife_csid_hw->hw_info->soc_info);
err:
	return rc;
}


int ais_tfe_csid_hw_deinit(struct ais_ife_csid_hw *ife_csid_hw)
{
	int rc = 0;

	if (ife_csid_hw) {
		ais_tfe_csid_deinit_soc_resources(
