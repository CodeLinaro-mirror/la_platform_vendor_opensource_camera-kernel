...
#include "ais_tfe_csid_soc.h"
#include "cam_cpas_api.h"
#include "cam_debug_util.h"

static int ais_tfe_csid_get_dt_properties(struct cam_hw_soc_info *soc_info)
{
...
static int ais_tfe_csid_request_platform_resource(
	...
	irq_handler_t csid_irq_handler, void *irq_data)
{
	...
	rc = ais_tfe_csid_get_dt_properties(soc_info);
	if (rc < 0)
		return rc;

	/* Need to see if we want post process the clock list */

	rc = ais_tfe_csid_request_platform_resource(soc_info, csid_irq_handler,
		irq_data);
	if (rc < 0) {
		CAM_ERR(CAM_ISP,
...
int ais_tfe_csid_deinit_soc_resources(
	struct cam_hw_soc_info *soc_info)
{
	int rc = 0;
	struct ais_csid_soc_private       *soc_private;
...
int ais_tfe_csid_enable_soc_resources(
	struct cam_hw_soc_info *soc_info, enum cam_vote_level clk_level)
{
	...
	rc = cam_soc_util_enable_platform_resource(soc_info,
			CAM_CLK_SW_CLIENT_IDX, true, clk_level, true);
	if (rc) {
