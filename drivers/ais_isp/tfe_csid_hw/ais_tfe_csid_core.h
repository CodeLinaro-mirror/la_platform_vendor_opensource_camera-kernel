...
#ifndef _AIS_TFE_CSID_HW_H_
#define _AIS_TFE_CSID_HW_H_

#include "cam_hw.h"
#include "ais_ife_csid_hw_intf.h"
#include "ais_tfe_csid_soc.h"

#define AIS_TFE_CSID_HW_RES_MAX      8
#define AIS_TFE_CSID_CID_RES_MAX     4
#define AIS_TFE_CSID_RDI_MAX         4
...
#define CSID_PATH_RDI_OVERFLOW_IRQ                BIT(17)
...
struct ais_ife_csid_common_reg_offset {
	...
	uint32_t format_measure_support;
...
struct ais_ife_csid_reg_offset {
	...
	const struct ais_ife_csid_rdi_reg_offset *rdi_reg[AIS_TFE_CSID_RDI_MAX];
...
struct ais_ife_csid_hw {
	...
	enum ais_isp_resource_state      state;
	struct ais_ife_csid_path_cfg     rdi_cfg[AIS_TFE_CSID_RDI_MAX];
	bool                             ipp_fix_cfg;
	...
	struct completion    csid_rdi_complete[AIS_TFE_CSID_RDI_MAX];
	uint64_t                         csid_debug;
	uint64_t                         clk_rate;
...
int ais_tfe_csid_hw_deinit(struct ais_ife_csid_hw *tfe_csid_hw);

#endif /* _AIS_TFE_CSID_HW_H_ */
