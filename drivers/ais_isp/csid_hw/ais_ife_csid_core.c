	...
	struct ais_ife_csid_hw *csid_hw)
{
	...
	input_core_sel = csid_reg->input_core_sel[csid_hw->hw_intf->hw_idx][1];
...
static int ais_ife_csid_ver_config_camif(
	struct ais_ife_csid_hw *csid_hw,
	struct ais_ife_csid_path_cfg *path_cfg, int id)
{
	struct cam_hw_soc_info                 *soc_info;
	const struct ais_ife_csid_reg_offset   *csid_reg;
	...
	cam_io_w_mb(path_cfg->epoch_cfg << csid_reg->rdi_reg[id]->epoch0_shift_val,
					soc_info->reg_map[0].mem_base +
					csid_reg->rdi_reg[id]->epoch_irq_cfg_addr);
	...
	struct ais_ife_rdi_init_args         *res)
{
	...
	uint32_t val = 0, cfg0 = 0, cfg1 = 0, id = 0, i = 0;
	...
	cfg1 = cam_io_r_mb(soc_info->reg_map[0].mem_base +
	...
	struct ais_ife_rdi_start_args   *start_cmd)
{
	...
	/* Enable the required RX RDI interrupts */
	val = 0x10186005;
	...
	val |= (0x1 << csid_reg->cmn_reg->buf_done_shitf_val) << id;
	cam_io_w_mb(val, soc_info->reg_map[0].mem_base +
					csid_reg->cmn_reg->csid_buf_done_irq_mask_addr);
...
static int ais_ife_csid_disable_rdi(
	struct ais_ife_csid_hw   *csid_hw,
	void *cmd_args)
{
	...
	const struct ais_ife_csid_reg_offset       *csid_reg;
	struct cam_hw_soc_info                     *soc_info;
	struct ais_ife_rdi_disable_rdi_args        *rdi_cmd;
	uint32_t val = 0;
	uint32_t id = 0;

	csid_reg = csid_hw->csid_info->csid_reg;
	soc_info = &csid_hw->hw_info->soc_info;
	rdi_cmd = (struct ais_ife_rdi_disable_rdi_args*)cmd_args;
	id = rdi_cmd->path;
	...
	/* disable rx RDI interruput*/
	cam_io_w_mb(0, soc_info->reg_map[0].mem_base +
		csid_reg->rdi_reg[id]->csid_rdi_irq_mask_addr);

	CAM_DBG(CAM_ISP, "CSID%d  disable rdi[%d]",
						csid_hw->hw_intf->hw_idx,
						id);
	return 0;
}
...
static irqreturn_t ais_ife_csid_irq(int irq_num, void *data)
{
	...
	if (irq_status[CSID_IRQ_STATUS_TOP] &
					CSID_TOP_IRQ_STATUS_BUF_DONE) {
		irq_status[CSID_IRQ_BUF_DONE] =
			cam_io_r_mb(soc_info->reg_map[0].mem_base +
			csid_reg->cmn_reg->csid_buf_done_irq_status_addr);
	}
	...
	cam_io_w_mb(irq_status[CSID_IRQ_BUF_DONE],
				soc_info->reg_map[0].mem_base +
				csid_reg->cmn_reg->csid_buf_done_irq_clear_addr);
...
handle_fatal_error:
	...
	if ((csid_hw->csid_debug & CSID_DEBUG_ENABLE_LONG_PKT_CAPTURE) &&
		...
		CAM_INFO_RATE_LIMIT(CAM_ISP,
			...
			csid_hw->hw_intf->hw_idx,
			((val >> 22) & 0x7C), ((val >> 16) & 0x3F), (val & 0xFFFF));
		val = cam_io_r_mb(soc_info->reg_map[0].mem_base +
		...
		CAM_INFO_RATE_LIMIT(CAM_ISP,
			...
			csid_hw->hw_intf->hw_idx, ((val >> 16) & 0xFFFF), (val & 0xFFFF));
	}
