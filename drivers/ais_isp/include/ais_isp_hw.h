...
enum ais_isp_hw_type {
	...
	AIS_ISP_HW_TYPE_TFE         = 4,
	AIS_ISP_HW_TYPE_TFE_CSID    = 5,
	AIS_ISP_HW_TYPE_MAX         = 6,
};
...
struct ais_ife_stream_config {
	uint32_t hw_idx;
	uint32_t csiphy_id;
	uint32_t num_rdi;
	struct ais_ife_rdi_init_args rdi_cfg[AIS_IFE_PATH_MAX];
};

struct ais_ife_stream_group_config_args {
	uint32_t num_hw;
	struct ais_ife_stream_config stream_cfg[AIS_IFE_HW_NUM_MAX];
};
