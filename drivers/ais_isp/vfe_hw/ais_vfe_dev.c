	...
	struct cam_hw_intf *csid_hw)
{
	...
	if (ais_vfe_hw_list[init->hw_idx]) {
		...
		struct ais_ife_csid_ver2_hw *csid_core = NULL;
		...
		csid_core = (struct ais_ife_csid_ver2_hw*)(csid_hw_info->core_info);
		csid_core->vfe_event_cb = ais_vfe_csid_irq_event_callback;
...
#else
void ais_vfe_remove(struct platform_device *pdev)
#endif
{
