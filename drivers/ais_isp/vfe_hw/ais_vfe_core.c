...
#define AIS_VFE_STATUS0_RDI_SOF_IRQ_SHFT 8
#define AIS_VFE_STATUS0_RDI_SOF_IRQ_MSK  0x1
...
#define AIS_VFE_STATUS0_BUS_WR_IRQ  (1 << 0)
#define AIS_VFE_STATUS0_RDI_SOF_IRQ  (0x15 << AIS_VFE_STATUS0_RDI_SOF_IRQ_SHFT)
#define AIS_VFE_STATUS0_RDI_OVERFLOW_IRQ  \
	(0xF << AIS_VFE_STATUS1_RDI_OVERFLOW_IRQ_SHFT)
#define AIS_VFE_STATUS0_RESET_ACK_IRQ  (1 << 0)
...
#define AIS_36BIT_INTF_GET_IOVA_BASE(iova) ((iova) >> 8)
#define AIS_36BIT_INTF_GET_IOVA_OFFSET(iova) ((iova) & 0xff)

#define AIS_VFE_ERR_IRQ_MASK0 						0x7

#define ALIGNUP(value, alignment) \
       ((value + alignment - 1) / alignment * alignment)
#define AIS_IFE_MAX_FIFO_CNT                        2
...
static int ais_vfe_bus_hw_init(struct ais_vfe_hw_core_info *core_info)
{
	...
	CAM_DBG(CAM_ISP, "VFE%d: hw bus version: 0x%x",
			core_info->vfe_idx,
			cam_io_r_mb(core_info->mem_base +
					bus_hw_info->common_reg.hw_version));
#if 0
	/*Set Debug Registers*/
...
static int ais_vfe_reset(void *hw_priv,
	void *reset_core_args, uint32_t arg_size, bool reset_skip)
{
...
static void ais_vfe_reset_rdi(void *hw_priv,
	enum ais_ife_output_path_id path, bool reset_skip)
{
	struct cam_hw_info *vfe_hw  = hw_priv;
	struct ais_vfe_top_ver2_hw_info *top_hw_info = NULL;
	struct ais_vfe_hw_core_info *core_info = NULL;
	...
	switch (rdi_cfg->out_cfg.format) {
		case CAM_FORMAT_MIPI_RAW_10:
			...
			if (rdi_cfg->out_cfg.mode == 1) {
				...
				/*line base mode*/
				rdi_path->en_cfg =  0x1;
				rdi_path->width = ALIGNUP((rdi_cfg->out_cfg.width), 8);
			}
			break;
...
int ais_vfe_init_hw(void *hw_priv, void *init_hw_args, uint32_t arg_size)
{
	...
	/* Do HW Reset */
	rc = ais_vfe_reset(hw_priv, &reset_core_args, sizeof(uint32_t), true);
	if (rc) {
...
int ais_vfe_reserve(void *hw_priv, void *reserve_args, uint32_t arg_size)
{
	...
	if (rdi_path->batchConfig.numBatchFrames < 1 ||
			rdi_path->batchConfig.numBatchFrames > 4) {
		CAM_ERR(CAM_ISP, "invalid numBatchFrames %d",
...
int ais_vfe_start(void *hw_priv, void *start_args, uint32_t arg_size)
{
	...
	cam_io_w_mb(core_info->irq_mask0,
		core_info->mem_base + top_irq_regs[0].mask_reg_offset);
	if (top_irq_reg_info->num_registers > 1)
...
int ais_vfe_stop(void *hw_priv, void *stop_args, uint32_t arg_size)
{
	...
	/* some hardware will skip reset*/
	if (core_info->vfe_hw_info->top_version == AIS_VFE_TOP_VER_4_0 &&
			core_info->vfe_hw_info->bus_version == AIS_VFE_BUS_VER_3_0) {
		reset_skip = true;
		goto _reset_skip;
	}
...
_reset_skip:
	if (rdi_path->state == AIS_ISP_RESOURCE_STATE_ERROR)
		ais_vfe_reset_rdi(vfe_hw, stop_cmd->path, reset_skip);
...
static void ais_vfe_q_bufs_to_hw_ver2(struct ais_vfe_hw_core_info *core_info,
		enum ais_ife_output_path_id path)
{
	...
	is_full = ((rdi_path->num_buffer_hw_q >= AIS_IFE_MAX_FIFO_CNT) ||
	...
	while (!is_full) {
		...
		cam_io_w_mb(iova_offset,
			core_info->mem_base + client_regs->image_addr_offset);
...
static int ais_vfe_flush_hw_q(struct ais_vfe_hw_core_info *core_info,
		...
		struct ais_ife_enqueue_buffer_args *enq_buf)
{
	...
	if (rc) {
		...
		if (rdi_path->state < AIS_ISP_RESOURCE_STATE_STREAMING)
			ais_vfe_q_bufs_to_hw_ver2(core_info, enq_buf->path);
	}
	...
	void *cmd_args, uint32_t arg_size)
{
	...
	case AIS_VFE_CMD_FLUSH_HW_Q: {
		...
		if (arg_size != sizeof(*flush))
			...
			rc = ais_vfe_flush_hw_q(core_info, flush);
		break;
	}
	default:
	...
	struct ais_vfe_hw_work_data *work_data)
{
	...
	for (path = 0; path < AIS_IFE_PATH_MAX; path++) {
		...
		//enq buffers
		spin_lock_bh(&p_rdi->buffer_lock);
		ais_vfe_q_bufs_to_hw_ver2(core_info, path);
		spin_unlock_bh(&p_rdi->buffer_lock);
	}
	...
	struct ais_vfe_hw_work_data *work_data)
{
	...
	struct ais_vfe_bus_ver2_reg_offset_bus_client  *client_regs = NULL;
	struct ais_vfe_rdi_output *p_rdi;
	struct ais_ife_rdi_disable_rdi_args rdi_cmd = {0};
	int path =  0;
	int rc = 0;
	...
	uint32_t last_addr)
{
	...
	while (rdi_path->num_buffer_hw_q && !last_addr_match) {
		...
		if (last_addr == AIS_36BIT_INTF_GET_IOVA_BASE(vfe_buf->iova_addr))
			last_addr_match = true;
		else
			CAM_WARN(CAM_ISP, "IFE%d RDI%d buf %d did not match addr",
				core_info->vfe_idx, path, vfe_buf->bufIdx);

		CAM_DBG(CAM_ISP, "I%d|R%d BUF DQ %d (0x%x) FIFO:%d|0x%x",
			core_info->vfe_idx, path,
			vfe_buf->bufIdx, AIS_36BIT_INTF_GET_IOVA_BASE(vfe_buf->iova_addr),
...
{
	struct ais_vfe_bus_ver2_hw_info   *bus_hw_info = NULL;
	struct ais_vfe_bus_ver2_reg_offset_common *common_reg = NULL;
	...
	bus_hw_info = core_info->vfe_hw_info->bus_hw_info;
	common_reg = &bus_hw_info->common_reg;
	...
	for (client = 0 ; client < AIS_IFE_PATH_MAX; client++) {
		...
		if (client_mask &
				(0x1 << (common_reg->buf_done_shitf_val + client))) {
	...
	uint32_t *status, int rdi_num)
{
	...
	work_data->bus_wr_status[0] = status[CSID_VER2_IRQ_STATUS_TOP];
	work_data->bus_wr_status[1] = status[CSID_VER2_IRQ_STATUS_BUF_DONE];
	work_data->bus_wr_status[2] = status[CSID_VER2_IRQ_STATUS_RX];
	...
	struct ais_vfe_hw_work_data *work_data)
{
	...
	if (vfe_hw_info->top_version != AIS_VFE_TOP_VER_4_0 &&
		vfe_hw_info->bus_version != AIS_VFE_BUS_VER_3_0) {
...
static int ais_vfe_buf_done(struct ais_csid_ver2_irq_event *irq_event)
{
	...
	vfe_hw = (struct cam_hw_info*)irq_event->priv;
	core_info = (struct ais_vfe_hw_core_info *)vfe_hw->core_info;
...
static int ais_vfe_sof_done(struct ais_csid_ver2_irq_event *irq_event)
{
	...
	vfe_hw = (struct cam_hw_info*)irq_event->priv;
	core_info = (struct ais_vfe_hw_core_info *)vfe_hw->core_info;
	csid_reg = irq_event->csid_hw->csid_info->csid_reg;
	dbg_bit_pos = csid_reg->path_debug_mask->bit_pos;

	for (i = 0; i < irq_event->rdi_num; i++) {
		path_data = &irq_event->csid_hw->rdi_cfg[i];
		if (!(irq_event->status[CSID_VER2_IRQ_STATUS_RDI0 + i] &
...
irqreturn_t ais_vfe_csid_irq_event_callback(struct ais_csid_ver2_irq_event *irq_event)
{
	switch (irq_event->event) {
		case AIS_CSID_VER2_IRQ_EV_SOF:
			ais_vfe_sof_done(irq_event);
			break;
		case AIS_CSID_VER2_IRQ_EV_BUF_DONE:
			ais_vfe_buf_done(irq_event);
			break;
		default:
			CAM_DBG(CAM_ISP, "Don't support the event %d", irq_event->event);
			break;
	}
...
static void cam_req_mgr_process_workq_cam_vfe_worker(struct work_struct *w)
{
	...
	struct ais_vfe_hw_info                     *vfe_hw_info)
{
	...
	rc = cam_req_mgr_workq_create(worker_name,
		...
		cam_req_mgr_process_workq_cam_vfe_worker);
	if (rc) {
