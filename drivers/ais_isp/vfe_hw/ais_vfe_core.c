...
#define AIS_VFE_IRQ_CMD 0x30
#define AIS_VFE_IRQ_MASK0 0x34
#define AIS_VFE_IRQ_MASK1 0x38
#define AIS_VFE_IRQ_CLEAR0 0x3C
#define AIS_VFE_IRQ_CLEAR1 0x40
#define AIS_VFE_IRQ_STATUS0 0x44
#define AIS_VFE_IRQ_STATUS1 0x48
		...
		enum ais_ife_output_path_id path)
{
	...
	while (!is_full) {
		...
		CAM_DBG(CAM_ISP, "IFE%d|RDI%d: Q %d(0x%x) FIFO:%d flush %d",
			core_info->vfe_idx, path,
		...
		struct ais_ife_enqueue_buffer_args *enq_buf)
{
	...
	rc = cam_mem_get_io_buf(vfe_buf->mem_handle,
		mmu_hdl, &vfe_buf->iova_addr, &src_buf_size, NULL);
	if (rc < 0) {
		CAM_ERR(CAM_ISP,
	...
	uint32_t *status, int rdi_num)
{
	...
	work_data->bus_wr_status[0] = status[CSID_IRQ_STATUS_TOP];
	work_data->bus_wr_status[1] = status[CSID_IRQ_BUF_DONE];
