...
static int ais_vfe_get_dt_properties(struct cam_hw_soc_info *soc_info)
{
	...
	soc_private->is_ife_lite = false;
	if (strnstr(soc_info->compatible, "lite",
				strlen(soc_info->compatible)) != NULL)
		soc_private->is_ife_lite = true;
	...
	irq_handler_t vfe_irq_handler, void *irq_data)
{
	...
	rc = cam_soc_util_get_option_clk_by_name(soc_info,
		AIS_VFE_DSP_CLK_NAME, &soc_private->dsp_clk_index);
	if (rc)
...
disable_platform_resource:
	if (cam_soc_util_disable_platform_resource(soc_info, CAM_CLK_SW_CLIENT_IDX, true, true))
