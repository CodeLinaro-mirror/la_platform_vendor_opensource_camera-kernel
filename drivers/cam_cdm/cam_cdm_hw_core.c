...
irqreturn_t cam_hw_cdm_irq(int irq_num, void *data)
{
	...
	for (i = 0; i < cdm_core->offsets->reg_data->num_bl_fifo_irq; i++) {
		...
		payload[i]->worker_scheduled_ts = ktime_get();
	...
	if (cdm_core->rst_done_cnt == cdm_core->offsets->reg_data->num_bl_fifo_irq) {
	...
	if (cdm_core->rst_done_cnt &&
		(cdm_core->rst_done_cnt != cdm_core->offsets->reg_data->num_bl_fifo_irq))
		CAM_INFO(CAM_CDM,
			"%s%u Reset IRQ received for %d fifos instead of %d",
			soc_info->label_name,
			soc_info->index, cdm_core->rst_done_cnt,
	...
	uint32_t            handle)
{
	...
	for (i = 0; i < cdm_core->offsets->reg_data->num_bl_fifo; i++)
		if (atomic_read(&cdm_core->bl_fifo[i].work_record)) {
			CAM_WARN(CAM_CDM,
				"workqer got delayed for %s%u, work_record :%u",
				soc_info->label_name, soc_info->index,
