...
#define HDMI_BDG_IRQ_LT6911UXE_IOCTL_GETRES        0x01
#define HDMI_BDG_IRQ_LT6911UXE_IOCTL_GETRES_DIR    0x02
#define HDMI_BDG_IRQ_LT6911UXE_IOCTL_RST           0x03
#define HDMI_BDG_IRQ_LT6911UXE_IOCTL_UPGRADE_FW    0x04

struct hdmi_bdg_lt6911uxe_res_info {
	int32_t width;
	int32_t height;
...
#define HDMI_BDG_IRQ_HANDLER_IOCTL_CMD_GETRES   \
	_IOR(HDMI_BDG_IRQ_LT6911UXE_MAGIC_NUM,        \
	HDMI_BDG_IRQ_LT6911UXE_IOCTL_GETRES, \
			struct hdmi_bdg_lt6911uxe_res_info)
#define HDMI_BDG_IRQ_HANDLER_IOCTL_CMD_GETRES_DIR   \
	_IOR(HDMI_BDG_IRQ_LT6911UXE_MAGIC_NUM,            \
	HDMI_BDG_IRQ_LT6911UXE_IOCTL_GETRES_DIR, \
			struct hdmi_bdg_lt6911uxe_res_info)
#define HDMI_BDG_IRQ_HANDLER_IOCTL_CMD_RST   \
	_IO(HDMI_BDG_IRQ_LT6911UXE_MAGIC_NUM,      \
	HDMI_BDG_IRQ_LT6911UXE_IOCTL_RST)
#define HDMI_BDG_IRQ_HANDLER_IOCTL_CMD_UPGRADE_FW   \
	_IO(HDMI_BDG_IRQ_LT6911UXE_MAGIC_NUM,      \
	HDMI_BDG_IRQ_LT6911UXE_IOCTL_UPGRADE_FW)


static const struct  of_device_id hdmi_bdg_irq_lt6911uxe_dev_id[] = {
	{ .compatible = "hdmi_bdg_lt6911uxe_handler" },
	{},
};
...
static struct hdmi_bdg_lt6911uxe_res_info s_hdmi_bdg_res_info;
...
MODULE_DEVICE_TABLE(of, hdmi_bdg_irq_lt6911uxe_dev_id);

static int hdmi_bdg_irq_lt6911uxe_dev_open(struct inode *inodp, struct file *filp)
{
	return 0;
}
static int hdmi_bdg_irq_lt6911uxe_dev_release(struct inode *inodp,
		struct file *filp)
{
	wake_up_all(&hdmi_bdg_read_wq);
	return 0;
}
static ssize_t hdmi_bdg_irq_lt6911uxe_dev_read(struct file *filp,
...
static long hdmi_bdg_irq_lt6911uxe_dev_ioctl(struct file *filp,
		unsigned int cmd, unsigned long arg)
{
	...
	case HDMI_BDG_IRQ_HANDLER_IOCTL_CMD_GETRES_DIR:
		CAM_INFO(CAM_SENSOR, "LT6911 firmare version: 0x%08x", cam_hdmi_bdg_lt6911uxe_get_fw_version());
		rc = cam_hdmi_bdg_lt6911uxe_get_src_resolution(
	...
	case HDMI_BDG_IRQ_HANDLER_IOCTL_CMD_UPGRADE_FW:
		...
		rc = cam_hdmi_bdg_lt6911uxe_upgrade_firmware();
...
#ifdef CONFIG_COMPAT
static long hdmi_bdg_irq_lt6911uxe_dev_compat_ioctl(struct file *filp,
		unsigned int cmd, unsigned long arg)
{
	return hdmi_bdg_irq_lt6911uxe_dev_ioctl(filp, cmd,
		(unsigned long)compat_ptr(arg));
}
#endif

static const struct file_operations hdmi_bdg_irq_lt6911uxe_dev_fops = {
	.owner = THIS_MODULE,
	.read = hdmi_bdg_irq_lt6911uxe_dev_read,
	.open = hdmi_bdg_irq_lt6911uxe_dev_open,
	.unlocked_ioctl = hdmi_bdg_irq_lt6911uxe_dev_ioctl,
#ifdef CONFIG_COMPAT
	.compat_ioctl = hdmi_bdg_irq_lt6911uxe_dev_compat_ioctl,
#endif
	.release = hdmi_bdg_irq_lt6911uxe_dev_release,
};

static irqreturn_t hdmi_bdg_lt6911uxe_irq_handler(int irq, void *p)
{
	int rc = 0;
...
static int hdmi_bdg_irq_lt6911uxe_probe(struct platform_device *pdev)
{
	int ret = 0;
	...
	ret = request_threaded_irq(hdmi_bdg_irq, NULL,
			hdmi_bdg_lt6911uxe_irq_handler,
			IRQF_TRIGGER_FALLING | IRQF_ONESHOT,
			"hdmi_bdg_lt6911uxe_irq", NULL);
	if (ret) {
	...
	if (ret)
		goto fail_cdev_del;
	hdmi_bdg_irq_handler_class = class_create(HDMI_BDG_IRQ_LT6911UXE_DEVNAME);
...
void hdmi_bdg_irq_lt6911uxe_remove(struct platform_device *pdev)
{
...
static struct platform_driver hdmi_bdg_lt6911uxe_handler_drv = {
	.probe = hdmi_bdg_irq_lt6911uxe_probe,
	.remove = hdmi_bdg_irq_lt6911uxe_remove,
	.suspend = NULL,
	.resume = NULL,
	.driver = {
		.name = HDMI_BDG_IRQ_LT6911UXE_DEVNAME,
		.of_match_table = of_match_ptr(hdmi_bdg_irq_lt6911uxe_dev_id),
	},
};

int hdmi_bdg_lt6911uxe_irq_handler_init(void)
{
	return platform_driver_register(&hdmi_bdg_lt6911uxe_handler_drv);
}

void hdmi_bdg_lt6911uxe_irq_handler_exit(void)
{
	platform_driver_unregister(&hdmi_bdg_lt6911uxe_handler_drv);
}

MODULE_DESCRIPTION("HDMI lt6911uxe driver");
