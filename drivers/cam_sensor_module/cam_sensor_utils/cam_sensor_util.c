	...
	struct cam_hw_soc_info *soc_info, struct completion *i3c_probe_status)
{
	...
	if (!ctrl->gpio_table_requested) {
		rc = cam_sensor_util_request_gpio_table(soc_info, 1);
		if (rc < 0) {
			CAM_ERR(CAM_SENSOR_UTIL, "GPIO table request failed");
			ctrl->gpio_table_requested = false;
		} else {
			ctrl->gpio_table_requested = true;
		}
	}
	...
	struct cam_sensor_power_ctrl_t *pwr_info)
{
	...
	if (!probe_cmd) {
		CAM_ERR(CAM_SENSOR_UTIL, "Invalid Args: probe_cmd %pK",
			probe_cmd);
		return -EINVAL;
	}
	...
	if (pwr_cfg->size_up > MAX_POWER_CONFIG ||
		...
		CAM_ERR(CAM_SENSOR_UTIL, "Invalid Args: size_up %d size_down %d",
	...
	CAM_DBG(CAM_SENSOR_UTIL, "power up/down sizes %d/%d",
	...
	for (i = 0; i < pwr_info->power_setting_size; i++) {
		...
		CAM_DBG(CAM_SENSOR_UTIL, "power up[%d] %d,%d,%d",
				i,
				pwr_info->power_setting[i].seq_type,
				pwr_info->power_setting[i].config_val,
				pwr_info->power_setting[i].delay);
	...
	for (i = 0; i < pwr_info->power_down_setting_size; i++) {
		...
		CAM_DBG(CAM_SENSOR_UTIL, "power down[%d] %d,%d,%d",
				i,
				pwr_info->power_down_setting[i].seq_type,
				pwr_info->power_down_setting[i].config_val,
				pwr_info->power_down_setting[i].delay);
	...
	struct cam_sensor_power_seq_array *pwr_info)
{
	...
	if (pwr_cfg->power_seq_size > MAX_POWER_CONFIG) {
		CAM_ERR(CAM_SENSOR_UTIL, "Invalid Args: size_down %d",
	...
	if (pwr_info->power_seq_size == 0) {
		CAM_WARN(CAM_SENSOR_UTIL, "Power sequence empty");
	} else {
		pwr_info->power_seq =
	...
	for (i = 0; i < pwr_info->power_seq_size; i++) {
		...
		CAM_DBG(CAM_SENSOR_UTIL, "power up/down[%d] %d,%d,%d",
				i,
				pwr_info->power_seq[i].seq_type,
				pwr_info->power_seq[i].config_val,
				pwr_info->power_seq[i].delay);
	...
	struct cam_sensor_power_setting *power_setting)
{
	...
	if (!gpio_conf) {
		CAM_INFO(CAM_SENSOR_UTIL, "No GPIO data");
	} else {
		...
		if (gpio_conf->cam_gpio_common_tbl_size <= 0) {
			CAM_INFO(CAM_SENSOR_UTIL, "No GPIO entry");
			no_gpio = -EINVAL;
		}
	...
	if (!power_setting) {
		CAM_ERR(CAM_SENSOR_UTIL, "Invalid power up settings");
		return -EINVAL;
	}
	...
	case SENSOR_MCLK:
		...
		if (power_setting->config_val) {
			...
			CAM_ERR(CAM_SENSOR_UTIL, "Invalid config value for mclk");
		break;
	case SENSOR_RESET:
	case SENSOR_STANDBY:
	case SENSOR_CUSTOM_GPIO1:
		...
		if (power_setting->seq_val < num_vreg) {
			if (power_setting->config_val == 1) {
				...
				if (msm_cam_sensor_handle_reg_gpio(power_setting->seq_type,
					gpio_num_info, 1) < 0) {
					CAM_ERR(CAM_SENSOR_UTIL, "Error in handling VREG GPIO");
					goto power_up_failed;
				}
				...
				if (msm_cam_sensor_handle_reg_gpio(power_setting->seq_type,
					...
					CAM_ERR(CAM_SENSOR_UTIL, "Error disabling VREG GPIO");
			} else
				CAM_ERR(CAM_SENSOR_UTIL, "Invalid config val");
		} else {
			CAM_ERR(CAM_SENSOR_UTIL, "usr_idx:%d dts_idx:%d",
				power_setting->seq_val, num_vreg);
		}
