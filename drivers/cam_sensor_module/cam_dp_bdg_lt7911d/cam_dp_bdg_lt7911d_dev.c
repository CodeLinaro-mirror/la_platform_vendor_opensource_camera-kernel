...
static int dp_bdg_ctl_rst_irq_regist(struct platform_device *pdev)
{
	...
	dp_ctl_rst.dp_ctl_rst_gpio= of_get_named_gpio(pdev->dev.of_node,
			"dp_ctl_rst_pin", 0);
...
static int dp_bdg_irq_lt7911d_probe(struct platform_device *pdev)
{
	...
	if (ret)
		goto fail_cdev_del;
	dp_bdg_irq_handler_class = class_create(DP_BDG_IRQ_LT7911D_DEVNAME);
...
void dp_bdg_irq_lt7911d_remove(struct platform_device *pdev)
{
