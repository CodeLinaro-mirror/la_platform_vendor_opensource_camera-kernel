	...
	case CAM_SENSOR_PROBE_CMD: {
		...
		if (s_ctrl->i2c_data.poweron_reg_settings.is_settings_valid) {
			...
			if (rc < 0) {
				...
				if (strcmp(DP_SENSOR_LT7911D_NAME, s_ctrl->io_master_info.client->name) &&
			...
			if (!strcmp(HDMI_BDG_LT6911UXE_NAME,s_ctrl->io_master_info.client->name))
			{
			...
			if (!strcmp(DP_SENSOR_LT7911D_NAME,s_ctrl->io_master_info.client->name))
			{
				cam_dp_bdg_lt7911d_set_cam_ctrl(s_ctrl);
			}
