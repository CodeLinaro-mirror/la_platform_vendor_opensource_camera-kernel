...
static int cam_sensor_exec_power_down_settings(struct cam_sensor_ctrl_t *s_ctrl)
{
	int rc = 0;
	struct cam_sensor_power_ctrl_t *power_info;
	struct cam_hw_soc_info *soc_info;
	...
	if (!s_ctrl || !s_ctrl->sensordata) {
        CAM_ERR(CAM_SENSOR, "Invalid parameters: s_ctrl %pK", s_ctrl);
        return -EINVAL;
    }
	...
	CAM_INFO(CAM_SENSOR, "powering down %d",
			s_ctrl->soc_info.index);

	for (index = 0; index < power_info->power_down_setting_size; index++)
	{
		rc = cam_sensor_util_power_apply(power_info, soc_info,
			&power_info->power_down_setting[index]);
		if (rc)
			CAM_ERR(CAM_SENSOR, "Failed to apply power down sequence");
	}
...
void cam_sensor_shutdown(struct cam_sensor_ctrl_t *s_ctrl)
{
	...
	if (s_ctrl->sensor_state != CAM_SENSOR_INIT) {
#ifdef CONFIG_MSM_AIS
		cam_sensor_exec_power_down_settings(s_ctrl);
#endif
...
#ifdef CONFIG_MSM_AIS
	rc = ais_sensor_power_up(power_info, soc_info, i3c_probe_completion);
#else
...
#ifdef CONFIG_MSM_AIS
	rc = ais_sensor_power_down(power_info, soc_info);
#else
